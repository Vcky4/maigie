...
> **Status:** In progress · **Phases 1–7 complete, Phase 8 substantially complete** (foundation, seasons, intake, triage, admin dashboard, reward ledger, pass redemption, cash payouts, participant web app, programme email, analytics and season reporting) · **no open product questions: the pass premium is settled as a duration bonus (§5.3, migration `084`)** · **M6 (Launch) waits only on the things that need a human: `083`+`084` against production, DNS for `issues.maigie.com`, season dates confirmed, the staging walkthrough with a real ₦1,000 transfer, and a triage rota** · updated 2026-09-13
> **Migration state:** `083_bug_hunt` **applied to staging 2026-09-12** — 8 tables, 3 partial unique indexes, 34 CHECK constraints, 22 foreign keys; 1 212 existing `User` rows untouched, since the migration is additive only (8 `CREATE TABLE`, 24 `CREATE INDEX`, no `DROP`, `ALTER` or `EXECUTE`). Read-only smoke test against the real schema returned `{"state":"between"}` from `GET /bug-hunt/program`. `084_bug_hunt_pass_bonus` **applied to staging 2026-09-13** — three statements in one transaction, renaming `passUpliftPercent` to `passBonusPercent` and moving its CHECK ceiling 90 → 100; no data migrated, because no `pass_redemption` ledger row existed anywhere. Staging is now at `084` with Season 1 `open`. **Neither migration is applied to production**
...
| B | Pass-over-cash premium | **Yes, 25% — and as a *duration bonus*, not a price discount. Revised 2026-09-13, migration `084`.** A tester pays the full catalogue price and is granted 25% more pass: ₦1,500 buys the 7-day pass and receives 9 days. Some premium is necessary because cash is fungible and a pass is not, so at parity a rational tester always takes cash. But the first version discounted the *price*, which handed over the pass **and** still owed the tester the difference: ₦1,500 buying a ₦1,125 pass leaves ₦375 of cash liability, converting a COGS cost into a naira one and stranding a stub below the ₦1,000 withdrawal minimum. Paying the premium in duration extinguishes the whole balance and costs us the cheapest thing we have. Per-season on `passBonusPercent`, so it can be tuned or turned off |
...
> **Resolved 2026-09-13, migration `084`: the premium is a duration bonus, not a price discount.** The original 25% uplift discounted the *price*, so a ₦1,500 balance bought a ₦1,500 pass for ₦1,125 and kept ₦375. That handed over the pass **and** still owed naira: a COGS cost converted into a cash one, plus a stub below the ₦1,000 withdrawal minimum that reads to a tester as money they earned and cannot reach. A premium is still necessary, because cash is fungible and a pass is not, so at parity a rational tester always takes the cash. It now lives in duration: full catalogue price, and 25% more pass. **₦700 buys 7 hours, ₦1,500 buys 9 days, ₦7,200 buys 5 months.** The balance is fully extinguished, the tester gets more than any Paystack customer, and the extra days cost compute.
>
> Two details settled in implementation. **The bonus scales the allowance as well as the clock** — a 9-day pass carrying 7 days of units stops working on day seven, which would make the duration bonus a promise the product does not keep. And **rounding is up, to the unit the pass is sold in**: 8.75 days becomes 9 days rather than 8 days 18 hours, because an offer has to be sayable. The consequence is that short passes get a larger effective bonus (5 hours to 7 is 40%), accepted because an hour is a coarse unit at that scale and the overshoot costs minutes of compute on the cheapest pass we grant.
>
> `passUpliftPercent` was **renamed** to `passBonusPercent` rather than added alongside it. This domain normally refuses to reinterpret a column a closed season claims it honoured — that is why the reward matrix lives on the row — but nothing was misrepresented: no season had closed and `BugHuntLedgerEntry` held zero `pass_redemption` rows on any environment, verified before the migration was written. The ceiling moved 90 → 100, since 100% now means a doubled pass rather than a free one.

**Plus pass.** Priced from config in kobo — `PRICE_NGN_PLUS_PASS_5H` (₦700), `PRICE_NGN_PLUS_PASS_7D` (₦1,500), `PRICE_NGN_PLUS_PASS_TERM` (₦7,200) — read at redemption time via a helper rather than duplicated, so a price change does not leave the programme selling at a stale figure. **The balance is charged the full price**; Decision B's 25% is added to what is granted: 5 hours → 7 hours, 7 days → 9 days, 4 months → 5 months, with the units allowance scaled to match.
...
- `GET /wallet/redemption-options` — pass products with the charged balance (the full catalogue price) and the bonused duration from the current season's `passBonusPercent`. **Available whether or not a season is open** — an earned balance is spendable in the gap between seasons.
...
- `GET /programs`, `GET /programs/{id}`, `POST /programs` *(super)*, `PATCH /programs/{id}` *(super)*. `POST` takes dates, budget, cap, country allowlist, reward matrix and pass bonus, defaulted from the previous season so opening Season 2 is a form with the last season's numbers already in it
...
**The season editor is what makes Season 2 a form rather than a release.** It creates a programme from the previous season's values — dates, budget, cap, country allowlist, reward matrix, pass bonus — with every field editable, then opens it, then offers the carry-forward action with a count of who would be seeded. If any part of opening a season requires an engineer, this page is not finished. Every queue on the other pages carries a season filter defaulting to the current one, so triage never silently spans two seasons.
...
- [x] Sign off the pass-over-cash premium (Decision B) — **25%, signed off 2026-09-12; revised 2026-09-13 from a price discount to a duration bonus, see §5.3.**
...
- [x] `services/redemption_service.py` — **prices read from `PRICE_NGN_PLUS_PASS_*`, the same constants Paystack charges against, so the programme cannot quietly sell a pass at a price the catalogue abandoned. **Revised by `084` (2026-09-13): the balance is charged the full catalogue price and the season's `passBonusPercent` is added to the granted duration and units instead, clamped at 100 (a doubled pass), falling back to the config default between seasons. Duration rounds up to the unit the pass is sold in, so the offer is sayable; units floor, because half a unit is not spendable.** 2026-09-12, revised 2026-09-13.**
...
- [x] Programme email notifications for the §4.3 points, including the season-opening send to past participants — **six templates plus the announcement, and the split between them is the design.** Outcome and money email (application decided, finding accepted, finding declined, cash approved, cash paid) goes **straight to the transport, bypassing the notification orchestrator**, exactly as `identity/emails.py` does: a tester who switched off study reminders has not asked to stop being told that ₦2,000 landed in their balance, and routing a statement about somebody's money through engagement policy would let a preference about revision nudges swallow it. The **season-opening announcement is the one exception** and goes through the orchestrator as `bug_hunt.season_open` under `OPERATIONS`, because it is the only Bug Hunt message that is genuinely marketing: a broadcast to a warm list who are owed nothing at the time of sending. That buys consent gating, an RFC 8058 unsubscribe header, quiet hours and retries for free, and the queue is also what stops one admin button press waiting on hundreds of provider round trips. A test asserts `bug_hunt.season_open` is the *only* `bug_hunt.*` type in the registry, so nobody can later move the payout receipt into the consent-gated path without reading why. **Nothing can fail its caller:** every send happens after the transaction that graded a finding or recorded a transfer, and a raise there would report a failed action, the operator would repeat it, and the repeat is the one that double-pays. Proved by making the transport raise rather than by reading the `except`. **Announcing is a separate act from opening a season**, with its own super-admin endpoint and a recipient count shown before the button does anything, because a send to a warm list cannot be recalled. 45 new tests. 2026-09-12.
- [x] Recruitment: `maigie-public` inbound link — **shipped in Phase 7** (landing section at position 11 plus a footer entry, target from `src/config/apps.ts`; see §8.2 for why the landing page rather than `/careers`). The announcement send itself is the Phase 8 email item above.
- [x] **Found while testing, and worth recording because it was live for the whole of Phase 8's first hour: the test suite was sending real email.** `.env` reaches `os.environ` as a side effect of collection, so `RESEND_API_KEY` was populated during test runs, and the moment triage started emailing outcomes `pytest tests/test_bug_hunt_reports.py` posted a dozen messages to Resend. They were refused with a 422 only because the fixtures use `@example.com` addresses; with plausible ones in a fixture it would have emailed real people from a test run. An autouse `no_outbound_email` fixture in `tests/conftest.py` now clears both providers for every test that does not explicitly request the `transport` fixture, which puts every sender on its documented skip-quietly path. Side benefit: the Bug Hunt suite went from 106s to 25s, because it had been waiting on live HTTP. 2026-09-12.
- [x] Analytics events (§13) — **the six client events, and the season report that makes them worth having.** `track()` in `apps/bughunt` forwards to `window.gtag` when the page has it and does nothing when it does not, with no analytics SDK added: a tester on Nigerian mobile data should not download one so we can count a page view. Every event carries `seasonNumber`, which is the entire requirement. `bughunt_apply_start` fires when the form is *shown to somebody who can use it* rather than on the landing page's button, because that button is also pressed by people who are then routed to a pending, ineligible or closed screen, and counting those as starts makes the start-to-submit drop-off unreadable. Backend `report_service` adds `GET /admin/bug-hunt/seasons/{id}/report` and `GET /admin/bug-hunt/retention`: cost per accepted finding **from the ledger** rather than from the budget, acceptance rate per platform, the `known_issue` rate reported separately because it is a metric about our backlog and not about testers, and season-over-season retention that counts people who **filed** rather than people who were carried forward — the latter would report 100% retention for a season nobody came back to, confidently, every season, forever. Every rate is `None` on an empty denominator, never `0`. 11 new tests. 2026-09-12.
- [x] Run `083` against staging, create Season 1 **through the admin season editor** rather than by SQL — **done, and verified from the audit trail rather than taken on trust.** `scripts/check_bug_hunt_season.py` reads the season row and reports it against `bug_hunt.rewards` defaults (against the module, not against numbers typed into the checker, so it cannot drift from what the domain considers correct). Season 1 is `open` on staging, 12–26 September, NG only, ₦300,000 budget, ₦15,000 cap, ₦1,000 minimum, +25% pass bonus, 10 a day, rules v1, and all six matrix amounts match. **The two `AuditLog` rows are the point**: `bug_hunt_create_season` at 20:49 and `bug_hunt_open_season` at 20:52, both through the admin API. That is Decision 12 demonstrated rather than asserted — Season 2 is a form somebody fills in, not a release. Had those rows been absent the season would still exist and the path to the next one would still be unproven, which is why the checker fails when it finds none. **Staging only: production has never had `083` applied**, so there is no season a real tester can reach. Zero participants and zero submissions, as expected while `issues.maigie.com` does not resolve. 2026-09-13.
...
- [ ] Open Season 1 — **note the staging window is already running** (closes 26 September). The dates were set when the season was created for testing, so if the real Season 1 is meant to start on deploy day they want moving in the editor rather than being allowed to expire during setup. A season whose `endsAt` has passed while still marked `open` keeps accepting intake, which the checker now flags.
...
| ~~Pass uplift percentage~~ | **Closed.** 25%, and paid as a duration bonus rather than a price discount (§5.3, migration `084`) |
...
| 2026-09-12 | **Phase 8 substantially shipped: programme email, analytics and season reporting. What remains for M6 needs a human, not an engineer.** Six email templates, `bug_hunt/emails.py`, `notify_service`, `report_service`, two admin report endpoints, the announcement console in `apps/admin`, six client analytics events, and 56 new tests. **The design decision worth defending is which mail bypasses consent.** Outcome and money email goes straight to the transport, exactly as `identity/emails.py` does: a tester who switched off study reminders has not asked to stop being told that ₦2,000 landed in their balance, and putting a statement about somebody's money behind an engagement preference lets a setting about revision nudges swallow it. The **season-opening announcement is the single exception**, registered as `bug_hunt.season_open` under `OPERATIONS`, because it is the only message here that is genuinely marketing: a broadcast to a warm list who are owed nothing when it is sent. That earns consent gating, an RFC 8058 unsubscribe header, quiet hours and retries for free, and its queue is also what stops one admin button press blocking on hundreds of provider round trips. A test asserts it is the **only** `bug_hunt.*` type in the registry, so the payout receipt cannot later be quietly moved behind a preference. **Nothing can fail its caller.** Every send happens after the transaction that graded a finding or recorded a bank transfer, so a raise would report a failed action, the operator would repeat it, and the repeat is the one that double-pays; proved by making the transport raise rather than by reading the `except` block. Announcing is deliberately a separate act from opening a season, with the recipient count shown before the button commits, because a send to a warm list cannot be recalled. **A real bug the tests caught, in the email itself:** the accepted-finding message was sending `creditedKobo`, which is **zero when an award is blocked by an exhausted budget** — so a critical bug would have been emailed as "Accepted: ₦0". It now sends the season's matrix amount with a paragraph saying the money is owed and being chased, because suppressing the email would leave the tester with no word on the outcome at all and ₦0 would simply be false. **A worse bug found while running the suite, and unrelated to Bug Hunt: the test suite was sending real email.** `.env` reaches `os.environ` during collection, so `RESEND_API_KEY` was populated, and the moment triage started emailing outcomes a test run posted a dozen live messages to Resend. They were refused with a 422 *only because the fixtures use `@example.com`*; with plausible addresses it would have emailed real people from `pytest`. An autouse `no_outbound_email` fixture in `tests/conftest.py` now clears both providers for any test that does not explicitly request the `transport` fixture. Side benefit: the Bug Hunt suite went from 106s to 25s, having been waiting on live HTTP. **On reporting, two numbers are easy to fake by accident and are asserted against faking:** cost per accepted finding comes from the **ledger**, not from the budget divided by a count, which would have reported ₦150,000 per finding against a ₦300,000 budget; and season-over-season retention counts people who **filed something**, not people who were carried forward, because carry-forward seeds everybody and counting the seeding would report 100% retention for a season nobody returned to, confidently, every season, forever. Every rate is `None` on an empty denominator rather than `0`, since 0% acceptance on the morning a season opens describes a catastrophe somebody would act on. The `known_issue` rate is reported as its own figure because it is a metric about **our backlog** rather than about testers. **Verification:** 583 Bug Hunt tests, full backend suite 5 215 passed, ruff and mypy clean on `src/domains/bug_hunt`, `nx build admin` green at the pre-existing 28-error baseline with zero Bug Hunt errors, `nx build bughunt` green. **Still open, and the last cheap moment to decide it: the pass premium shape (§5.3).** A 25% *price* discount leaves cash liability on the books for every pass redeemed; paying the premium in duration instead extinguishes the whole balance and costs compute rather than naira. Season 1 has not opened, so it is not yet a migration over live money |
| 2026-09-13 | **The pass premium changed shape: a duration bonus, not a price discount (migration `084`).** Raised by a look at the wallet screen — "why are we making this cheaper" — and the answer is that we should not be. A 25% *price* discount meant a ₦1,500 balance bought a ₦1,500 pass for ₦1,125 and kept ₦375: we handed over the pass, which costs us COGS, **and still owed the tester ₦375 in cash**. Every discounted redemption converted a compute cost into a naira one, and left a stub below the ₦1,000 withdrawal minimum that reads to a tester as money they earned and cannot reach. It also bought very little, because the pass-versus-cash decision is driven by whether somebody wants Plus at all rather than by the exchange rate — certain cost, speculative behaviour change. A premium is still necessary: cash is fungible and a pass is not, so at parity a rational tester always takes the cash and the programme pays real money on every accepted finding. So the premium moved into duration. **Full catalogue price, 25% more pass: ₦700 buys 7 hours, ₦1,500 buys 9 days, ₦7,200 buys 5 months.** The balance is fully extinguished, the tester gets more than any Paystack customer, and the extra days cost the cheapest thing we have. `pass_service.grant` already accepted a `duration_minutes` override and snapshots it on the row, so billing needed no change and the bonus is a durable property of the pass rather than something recomputed later from a season that may have changed its mind. **Two things settled while implementing, both of which the first draft got wrong.** The bonus **scales the units allowance as well as the clock** — a 9-day pass carrying 7 days of units stops working on day seven, which would make the duration bonus a promise the product does not keep; and the allowance is bonused from the **NGN** base of 4 500 rather than the global 10 000, or the pass would carry an entitlement this market does not sell. Rounding is **up, and to the unit the pass is sold in**: I first rounded to the hour, which turns 7 days at 25% into 8 days 18 hours — arithmetically honest and useless as copy, and it contradicted the "9 days" I had already promised. Rounding to the sold unit costs minutes of compute and buys a sentence that says nine days and means it. The accepted consequence is that short passes get a larger effective bonus (5 hours to 7 is 40%, because an hour is coarse at that scale). **`passUpliftPercent` was renamed rather than deprecated in place.** This domain normally refuses to reinterpret a column a closed season claims it honoured — that is the whole reason the reward matrix lives on the row — but nothing was misrepresented here: no season had closed and `BugHuntLedgerEntry` held **zero** `pass_redemption` rows on any environment, verified before the migration was written and repeatable with `scripts/check_bug_hunt_season.py`. A rename keeps one column meaning one thing instead of leaving a dead one for somebody to read as authoritative. The CHECK ceiling moved 90 → 100, since 100 now means a doubled pass rather than a free one, and the downgrade deliberately restores the stricter bound so a season configured at 91–100 would make the reversal *fail* rather than quietly turn somebody's configuration into a different offer. **Copy changed everywhere it was a discount claim**: the wallet now says "9 days instead of 7" and "+25% free" rather than a struck-through price and the confusing "25% further", because a comparison is checkable where a percentage is a sum the reader has to do — and the struck-through price made a paid programme read like a discount store. **Verification:** migration applied to staging and Season 1 re-checked (`+25% duration (full price charged)`, all six matrix amounts unchanged); 586 Bug Hunt tests, full backend suite 5 215 passed; ruff and mypy clean; `nx build bughunt` and `nx build admin` green with admin at its 28-error baseline. **Production still has neither `083` nor `084`**, which is what made this a schema change to a table with no real rows rather than a migration over live money |
