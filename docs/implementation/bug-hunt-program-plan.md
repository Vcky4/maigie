...
> **Status:** In progress · **Phases 1–7 complete, Phase 8 substantially complete** (foundation, seasons, intake, triage, admin dashboard, reward ledger, pass redemption, cash payouts, participant web app, programme email, analytics and season reporting) · **M6 (Launch) now waits only on the things that need a human: DNS for `issues.maigie.com`, Season 1 created through the admin editor, the staging walkthrough with a real ₦1,000 transfer, and a triage rota** · **one open product question: whether the pass premium should be a price discount or a duration bonus (§5.3)** · updated 2026-09-12
...
> **Open, and cheapest to change now: the pass premium is the wrong shape.** The 25% uplift is a *discount on the price*, so a ₦1,500 balance buys a ₦1,500 pass for ₦1,125 and leaves ₦375 of cash liability on the books. We give away the pass **and** still owe naira. Some premium is necessary — cash is strictly more liquid than a pass, so at a 1:1 rate a rational tester always takes cash and we pay real money on every accepted finding — but the premium should be paid in **duration, not price**: charge the full ₦1,500 and grant 9 days instead of 7. The balance is fully extinguished, the tester gets more than anyone paying Paystack, and the extra days cost us compute rather than cash. It also stops the wallet accumulating sub-minimum stubs that read to a tester as money they cannot reach. `pass_service.grant` already accepts a `duration_minutes` override and snapshots it, so no billing change is needed; it wants a new season column rather than reinterpreting `passUpliftPercent`, for the same reason the reward matrix lives on the season row. **Season 1 has not opened, so this is the last moment it is not a migration over live money.** Not yet decided.
...
- [x] Run `083` against staging, create Season 1 **through the admin season editor** rather than by SQL — **done, and verified from the audit trail rather than taken on trust.** `scripts/check_bug_hunt_season.py` reads the season row and reports it against `bug_hunt.rewards` defaults (against the module, not against numbers typed into the checker, so it cannot drift from what the domain considers correct). Season 1 is `open` on staging, 12–26 September, NG only, ₦300,000 budget, ₦15,000 cap, ₦1,000 minimum, 25% uplift, 10 a day, rules v1, and all six matrix amounts match. **The two `AuditLog` rows are the point**: `bug_hunt_create_season` at 20:49 and `bug_hunt_open_season` at 20:52, both through the admin API. That is Decision 12 demonstrated rather than asserted — Season 2 is a form somebody fills in, not a release. Had those rows been absent the season would still exist and the path to the next one would still be unproven, which is why the checker fails when it finds none. **Staging only: production has never had `083` applied**, so there is no season a real tester can reach. Zero participants and zero submissions, as expected while `issues.maigie.com` does not resolve. 2026-09-13.
