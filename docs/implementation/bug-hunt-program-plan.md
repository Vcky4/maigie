...
> **Status:** In progress · **Phases 1–6 complete** (foundation, seasons, intake, triage, admin dashboard, reward ledger, pass redemption, cash payouts) · Phases 7–10 open · **M1–M5 shipped — the backend is feature-complete** · updated 2026-09-12
...
- [ ] `GET /banks` — Nigerian bank list, cached 24 h. Not built: the participant app does not exist yet, and a hardcoded list in the client is worse than a picker fed by Paystack's own list. Phase 7 needs it
...
- [ ] Participant-side withdrawal history on the wallet screen — the API is done and returns all four states; the screen belongs to `apps/bughunt` in Phase 7
