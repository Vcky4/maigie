...
- [x] Add push/click handlers and route allowlisting — **the worker owns no routing, which is the whole point. The app already had exactly one authority on where a notification leads: a closed allowlist that gates on action version, encodes id segments, and refuses dot-segments. A hand-written worker cannot import it, and reimplementing it there would have created a second copy that drifts, in the one place nobody looks. So on click the worker resolves nothing — it focuses an open page and hands over the action, or on a cold start opens `/notifications?open=<id>` and lets the page resolve it. The server is never trusted with a URL either: the push payload carries only the canonical action, so there is no field in which a redirect could arrive.**
    ...
    **16 new tests pin the resolver, which had none despite being a security boundary — including that no URL in an action ever becomes a destination, that an unknown action version is refused rather than optimistically interpreted, and that an id is encoded so it stays one segment. The settings screen's tests were rewritten around the two-halves rule; one of them previously asserted that no browser-push switch existed, which was correct then and is wrong now. 436 web tests and 4,340 backend tests pass, with `tsc` and ESLint clean at zero warnings. Not verified: the built `dist` output — the production build's prerender step needs a Puppeteer Chrome that is not installed locally, so the assets landing at their final URLs is confirmed by configuration review only (2026-09-01).**
- [ ] Validate Chrome, Edge, Firefox, Safari macOS, and installed iOS PWA behavior where supported. **The remaining gate, and it cannot be done from a test suite. Prerequisites: generate a VAPID pair per deployment with `scripts/generate_vapid_keys.py`, set `WEB_PUSH_VAPID_*`, publish the public half to the web client, then set `WEB_PUSH_ENABLED=true` with the operator in `WEB_PUSH_INTERNAL_ALLOWLIST` and rollout still at 0%. Worth exercising specifically: a notification arriving with every tab closed; a click cold-starting the app and landing on the resolved route rather than the notification centre; a dismissal appearing as a `DISMISSED` interaction after the next page load; revoking permission in browser settings while the tab is open; and the installed iOS PWA, where Safari only delivers push to an app added to the home screen and permission must be requested from a user gesture. The maskable icon should be checked on an Android launcher, since a wrong safe zone only shows up there.**
