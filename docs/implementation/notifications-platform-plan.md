...
- [x] Remove obsolete preference columns after one compatibility release — **Done (2026-09-03), as a measured, staged, consent-sensitive migration rather than a bare column drop. `UserPreferences.{notifications, emailMorningSchedule, emailScheduleReminder, emailWeeklyTips, pushScheduleReminder, pushStudyTips}` are gone (migration `078`). Getting there safely, in order: (1) **measured the delta** against the configured DB — 1,206 users, all already have a `NotificationPolicy`, and ~1,200 would have `learning.digest`/`progress.digest` become email-eligible (legacy email defaults were `true`), with **zero real-world effect today** because `NOTIFICATION_EMAIL_ENABLED` is off; the semantic (a category's non-exact email types follow the category setting) was signed off. (2) **Materialized** category-level `NotificationPreference` rows for every user (migration `077`) reproducing `_effective_category_setting` exactly — verified 1,206/1,206 covered, 18,095 rows, no constraint violations, effective settings unchanged. (3) **Rewired dispatch** off the redundant `legacy.notifications` master gate onto `policy.engagement_enabled` in all three dispatchers plus `_email_plan` and the digest-subscription query — verified **zero divergence** between the two across all users before removing it. (4) **Stopped the settings read and write from touching the columns** (removed the legacy fallbacks in `get_notification_settings`/`_effective_category_setting` and the `legacy_values` dual-write), and removed the vestigial email/push fields from the identity `/preferences` API (no client sends them — the web `updatePreferences` only carries `studyGoals` and has no callers). (5) **Dropped the columns.** `timezone`/`timezoneSource` on `UserPreferences` are retained (still the timezone source of truth). Verified end to end against PostgreSQL: ORM loads, settings read, and dispatch consent all work against the column-dropped schema. 4,643 backend tests, Ruff and format pass; web tsc clean after regenerating API types.**
