...
- [ ] Remove legacy `/learning/notifications` adapters after client adoption. — **Verified ready but deliberately held. Both live clients are fully on the canonical `/api/v1/notifications*` routes (web `notificationsApi.ts`, mobile `endpoints.notifications.*`); the only remaining references to `/learning/notifications` are generated OpenAPI artifacts, a mobile test fixture, and docs — no runtime callers. The block is not the code, it is deployed old mobile builds in the field that may still call the legacy path: removing a public endpoint is a client-min-version decision, not a safe unilateral cleanup, so it waits on a mobile version floor.**
...
- [ ] Retire the obsolete provider path and `pushedAt` semantics. — **Verified dead, retirement staged. Against the configured PostgreSQL: zero `Notification` rows in the legacy `PENDING`/`QUEUED` states (none created in the last 14 days — no producer writes them any more), and `pushedAt` is non-null nowhere. So the FCM sender (`shared/infrastructure/push_notifications.py`), the legacy `deliver_pending`/`_push` sweep, and the `pushedAt` write are confirmed dead code superseded by `NotificationDelivery` evidence. Not removed in this pass because it is a large destructive change — a still-scheduled beat task, the FCM module, and ~50 bound tests — with a production residual-drain consideration that deserves its own focused change and an explicit go-ahead rather than being bundled in.**
