...
- [x] Add optional LLM digest summarization/template selection on the heavy queue — **`digest_copy.resolve_digest_copy` can replace the digest's deterministic title/body with a one-paragraph summary the model writes from the same items, and `plan_due_digests` calls it. It is **off by default** (`NOTIFICATION_DIGEST_LLM_ENABLED=False`) and **shadow by default** when on (`NOTIFICATION_DIGEST_LLM_SHADOW_ONLY=True`), mirroring the decision engine's discipline: an enabled-but-shadow run generates, validates and records a proposal but the learner still receives the deterministic copy, so the model is measured against live traffic at zero risk until a human turns shadow off. A separate `DIGEST_LLM` capability gate from `INTELLIGENCE`, because the copy writer and the timing engine are different blast radii and must arm independently; same fail-closed cohort machinery. Cost is bounded (small `max_tokens`, a 20s per-call timeout) and metered through the existing per-operation usage path (`operation="notification_digest"`) inside `proactive_scope()`, so its spend lands in the learner's proactive sub-budget. One honest deviation from the item's wording: the digest planner runs on the `default` queue, not `heavy`; the LLM step is an inline, bounded, proactive-scoped call in the hourly planner rather than a separate heavy-queue task, which is sufficient at this volume and avoids a second scheduling path. Moving it to `heavy` is a follow-up if digest volume grows.**
