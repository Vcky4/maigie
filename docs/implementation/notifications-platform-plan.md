...
- [ ] Run staged physical-device delivery tests on iOS and Android — **development-build physical-device evidence was collected on 2026-08-31 for the single allowlisted user at 0% cohort rollout: Android and iOS each displayed foreground, background, and terminated-app notifications; all six platform-specific Expo tickets were accepted and receipts reconciled to `DELIVERED` in one attempt; background and cold-start taps reached study plan `2eb581f397434c299054e836b` and persisted exactly one platform-correct `OPENED` plus `ACTIONED` pair per tap. Observed development-client navigation delays were Android background 3–5s, Android cold start 6–7s after account loading, iOS background ~4s, and iOS cold start ~3s blank after bundling/account loading. This remains unchecked until the same matrix passes in an EAS preview/release build and online/offline logout revocation recovery is physically verified.**
- [ ] Enable for internal users, then 5%, 25%, 100% — **only a temporary local single-user allowlist was enabled for the 2026-08-31 smoke test; it was removed afterward, `MOBILE_PUSH_ENABLED` was restored to `false`, and deterministic rollout remains `0`. Do not advance cohorts until the remaining physical-device gates are recorded.**

Exit criteria: provider acceptance and receipt evidence exist; invalid tokens are disabled; no preference, quiet-hour, or duplicate-send violations occur. **Development-build provider/display/tap evidence now exists on physical Android and iOS, but Phase 2 release remains gated on preview/release-build repetition, physical logout/offline revocation recovery, and operational rollout review. Production rollout remains disabled at 0% (2026-08-31).**
