...
- [ ] Enable for internal users, then 5%, 25%, 100% — **the physical matrix used a temporary single-user staging allowlist while deterministic rollout remained `0`. Safe example defaults now keep the sender enabled with empty allowlists, 0% rollout, and a 900s receipt delay, so no users are eligible; staging must be redeployed with those restored eligibility controls before this validation session is closed. Do not advance cohorts without an explicit operational rollout review.**
