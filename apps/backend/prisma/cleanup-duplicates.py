    ...
    try:
        ...
        for user in all_users:
            if user.stripeCustomerId and user.stripeCustomerId in seen_customer_ids:
                print(f"Found duplicate stripeCustomerId: {user.stripeCustomerId} for user {user.id} ({user.email})")
        ...
        for user in all_users:
            if user.stripeSubscriptionId and user.stripeSubscriptionId in seen_subscription_ids:
                print(f"Found duplicate stripeSubscriptionId: {user.stripeSubscriptionId} for user {user.id} ({user.email})")
                await db.user.update(
                    where={"id": user.id},
