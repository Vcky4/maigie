...
class BulkScheduleRegenRequest(BaseModel):
    """Request body for bulk schedule regeneration."""

    max_users: int = Field(default=50, ge=1, le=500, description="Maximum users to process")
    only_inactive_days: int | None = Field(
        default=None,
        ge=1,
        description="Only regenerate for users inactive N+ days (None = all with courses)",
    )


@router.post("/users/regenerate-schedules", response_model=dict)
async def bulk_regenerate_schedules(
    ...
    This gives returning users fresh, relevant schedules instead of stale
    plans from weeks ago. Useful after outages or major updates.
    """
    from ..services.schedule_regeneration_service import regenerate_user_schedule
    ...
    where_clause: dict = {
        ...
        "isOnboarded": True,
        "role": "USER",
        "courses": {"some": {"archived": False}},
    }

    if body.only_inactive_days:
        cutoff = now - timedelta(days=body.only_inactive_days)
        where_clause["updatedAt"] = {"lt": cutoff}
    ...
    results = {"total": len(users), "regenerated": 0, "failed": 0, "skipped": 0}
    ...
    for user in users:
        try:
            ...
            logger.warning("Schedule regen failed for user %s: %s", user.id, e)
            results["failed"] += 1

    await log_admin_action(
        admin_user_id=admin_user.id,
        action="BULK_SCHEDULE_REGEN",
        resource_type="schedules",
        details=results,
    )

    logger.info(
        "Bulk schedule regeneration: %d/%d regenerated by admin %s",
        results["regenerated"],
        results["total"],
        admin_user.id,
    )
...
# ============================================================================
# Deep Wake Window Configuration
# ============================================================================


class DeepWakeConfigRequest(BaseModel):
    """Request body for updating deep-wake configuration."""

    max_inactive_days: int = Field(
        ge=14,
        le=180,
        description="Maximum days of inactivity to target (default 30, increase to reach older churned users)",
    )


class DeepWakeConfigResponse(BaseModel):
    max_inactive_days: int
    description: str


@router.get("/retention/deep-wake-config", response_model=DeepWakeConfigResponse)
async def get_deep_wake_config(admin_user: SuperAdminUser):
    """Get the current deep-wake inactivity window configuration."""
    from ..services import system_config_service

    value = await system_config_service.get_config("retention.deep_wake_max_days", "30")
    max_days = int(value)

    return DeepWakeConfigResponse(
        max_inactive_days=max_days,
        description=f"Deep wake targets users inactive 7-{max_days} days (runs Mon/Wed/Fri at 11 AM UTC)",
    )


@router.put("/retention/deep-wake-config", response_model=DeepWakeConfigResponse)
async def update_deep_wake_config(
    body: DeepWakeConfigRequest,
    admin_user: SuperAdminUser,
):
    """
    Update the deep-wake inactivity window.

    Increase max_inactive_days to reach users who've been gone longer.
    Default is 30 days. Set to 90-180 to recover users from a mass outage.
    Changes take effect on the next scheduled deep-wake run.
    """
    from ..services import system_config_service

    await system_config_service.set_config(
        key="retention.deep_wake_max_days",
        value=str(body.max_inactive_days),
        category="retention",
    ...
    await log_admin_action(
        ...
        details={"max_inactive_days": body.max_inactive_days},
    )

    logger.info(
        "Deep wake window updated to %d days by admin %s",
        body.max_inactive_days,
        admin_user.id,
    )

    return DeepWakeConfigResponse(
        max_inactive_days=body.max_inactive_days,
        description=f"Deep wake targets users inactive 7-{body.max_inactive_days} days (runs Mon/Wed/Fri at 11 AM UTC)",
    )
