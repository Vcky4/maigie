...
async def assign_seat(
    space_id: str, target_user_id: str, actor_user_id: str, **kwargs
) -> dict[str, Any]:
    ...
    if target_member is None:
        raise SeatServiceError(
            ...
            message="The target user is not a member of this space.",
            status_code=400,
        )
    ...
    if str(target_member.seat_tier) == "PLUS_SEAT":
        raise SeatServiceError(
            ...
            message="The target user already holds a Plus seat in this space.",
            status_code=409,
        )
    ...
    if assigned_count >= pool_size:
        raise SeatServiceError(
            ...
            message=f"No available Plus seats. {assigned_count}/{pool_size} seats are assigned.",
            status_code=409,
        )
    ...
    logger.info(
        "assign_seat: user_id=%s space_id=%s by actor=%s", target_user_id, space_id, actor_user_id
    )
...
async def unassign_seat(
    ...
    if str(target_member.seat_tier) != "PLUS_SEAT":
        raise SeatServiceError(
            ...
            message="The target user does not hold a Plus seat in this space.",
            status_code=409,
        )
    ...
    logger.info(
        "unassign_seat: user_id=%s space_id=%s by actor=%s", target_user_id, space_id, actor_user_id
    )
...
async def reassign_seat(
    space_id: str, from_user_id: str, to_user_id: str, actor_user_id: str, **kwargs
) -> dict[str, Any]:
    ...
    if from_member is None:
        raise SeatServiceError(
            code=TARGET_NOT_MEMBER,
            message="The source user is not a member of this space.",
            status_code=400,
        )
    if str(from_member.seat_tier) != "PLUS_SEAT":
        raise SeatServiceError(
            code=TARGET_DOES_NOT_HAVE_PLUS_SEAT,
            message="The source user does not hold a Plus seat in this space.",
            status_code=409,
        )
    ...
    if to_member is None:
        raise SeatServiceError(
            code=TARGET_NOT_MEMBER,
            message="The destination user is not a member of this space.",
            status_code=400,
        )
    if str(to_member.seat_tier) == "PLUS_SEAT":
        raise SeatServiceError(
            code=TARGET_ALREADY_HAS_PLUS_SEAT,
            message="The destination user already holds a Plus seat in this space.",
            status_code=409,
        )
    ...
    logger.info(
        "reassign_seat: from=%s to=%s space_id=%s by actor=%s",
        from_user_id,
        to_user_id,
        space_id,
        actor_user_id,
    ...
    return {
        ...
        "to": {
            "userId": updated_to.user_id,
            "spaceId": updated_to.space_id,
            "seatTier": str(updated_to.seat_tier),
        },
    }
