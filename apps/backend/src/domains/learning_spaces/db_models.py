...
class Space(Base, TimestampMixin):
    ...
    seat_pool_size: Mapped[int] = mapped_column(
        "seatPoolSize", Integer, default=0, server_default="0"
    )

    # Moderation
    hidden_by_moderation: Mapped[bool] = mapped_column(
        "hiddenByModeration", Boolean, default=False, server_default="false"
    )
    allow_member_export: Mapped[bool] = mapped_column(
        "allowMemberExport", Boolean, default=False, server_default="false"
    )
    ...
    members: Mapped[list["SpaceMember"]] = relationship(
        "SpaceMember", back_populates="space", lazy="selectin"
    )
    chat_groups: Mapped[list["SpaceChatGroup"]] = relationship(
        "SpaceChatGroup", back_populates="space", lazy="noload"
    )
...
class SpaceMember(Base):
    ...
    space_id: Mapped[str] = mapped_column(
        "spaceId", String, ForeignKey("Space.id", ondelete="CASCADE"), index=True
    )
    ...
    seat_tier: Mapped[str] = mapped_column(
        "seatTier", String, default="FREE_SEAT", server_default="FREE_SEAT"
    )
...
class SpaceChatGroupMember(Base):
    ...
    chat_group_id: Mapped[str] = mapped_column(
        "chatGroupId", String, ForeignKey("SpaceChatGroup.id", ondelete="CASCADE"), index=True
    )
    user_id: Mapped[str] = mapped_column(
        "userId", String, ForeignKey("User.id", ondelete="CASCADE"), index=True
    )
...
class SpaceMemberStat(Base, TimestampMixin):
    ...
    contribution_points: Mapped[int] = mapped_column(
        "contributionPoints", Integer, default=0, server_default="0"
    )
    courses_completed: Mapped[int] = mapped_column(
        "coursesCompleted", Integer, default=0, server_default="0"
    )
    notes_added: Mapped[int] = mapped_column("notesAdded", Integer, default=0, server_default="0")
    resources_shared: Mapped[int] = mapped_column(
        "resourcesShared", Integer, default=0, server_default="0"
    )
    quiz_average: Mapped[float] = mapped_column(
        "quizAverage", Float, default=0.0, server_default="0"
    )
...
class SpaceSession(Base, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(
        String, default="SCHEDULED", server_default="SCHEDULED", index=True
    )
...
class SpaceSubscription(Base, TimestampMixin):
    ...
    space_id: Mapped[str] = mapped_column(
        "spaceId", String, ForeignKey("Space.id", ondelete="CASCADE"), unique=True
    )
    provider: Mapped[str] = mapped_column(String, nullable=False)
    provider_subscription_id: Mapped[str] = mapped_column(
        "providerSubscriptionId", String, nullable=False
    )
    ...
    cancel_at_period_end: Mapped[bool] = mapped_column(
        "cancelAtPeriodEnd", Boolean, default=False, server_default="false"
    )
    owner_user_id: Mapped[str] = mapped_column(
        "ownerUserId", String, ForeignKey("User.id", ondelete="CASCADE")
    )
...
class SpaceSeatAddon(Base):
    ...
    purchased_by_user_id: Mapped[str] = mapped_column(
        "purchasedByUserId", String, ForeignKey("User.id", ondelete="CASCADE"), index=True
    )
...
class AiUsageRecord(Base):
    ...
    request_count: Mapped[int] = mapped_column(
        "requestCount", Integer, default=1, server_default="1"
    )
    created_at: Mapped[datetime] = mapped_column(
        "createdAt",
        DateTime(timezone=True),
