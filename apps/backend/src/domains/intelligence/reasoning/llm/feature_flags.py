...
Ported from ``src/services/llm/feature_flags.py`` at ``4953972^``, replacing a fail-closed stub.

**Why the stub could not stay.** It exposed only ``is_enabled`` and ``get_variant``, and neither is
what this class is actually for. ``router._select_candidates`` calls ``is_model_allowed`` and
``websocket_handler`` (lines 444 and 1553) calls ``effective_tier_for_request``; both would have
raised ``AttributeError``, the second of them *before* ``get_llm_router()`` could raise its
deliberate "unmigrated" error. So the documented failure mode was not the real one.

**Why fail-closed was the wrong shape here, even though it was right for a flag.** An unknown
feature flag reading as off keeps behaviour at the previous default, which is safe. An entitlement
allowlist reading as "no model is permitted for anyone" is not a safe default, it is an inert
subsystem — and it cannot be told apart from a misconfigured allowlist. The precedence chain below
already denies by default at step 3: a pair absent from the tier allowlist is denied. Configuration
decides access, not a stub.
...
- ``_fetch_seat_tier`` imported ``src.services.seat_service.get_seat_tier``, and a second
  ...
  which reads ``SpaceMember.seat_tier`` and already returns ``"FREE_SEAT"`` for non-members and
  lookup failures — the same contract the legacy docstring promised. Two paths to one fact became
  one.

**The ``circle:`` scope prefix is deliberately not renamed.** ``Space`` replaced ``Circle`` in the
models, but this string is a scope value that ``emit_ai_usage`` persists and that
``websocket_handler`` builds with ``circle_scope(circle_group.space_id)``. Renaming the prefix would
split usage history across two spellings for the same scope. The id inside it is a space id, and the
...
# against the User's Seat_Tier in that Space. The two scopes are
# mutually isolated (Requirements 7.2, 7.3, 7.4).
UsageScope = str  # "personal" | "circle:{space_id}"

PERSONAL_SCOPE: UsageScope = "personal"


def circle_scope(space_id: str) -> UsageScope:
    """Build a space-scoped Usage_Scope value for a given space id.

    The ``circle:`` prefix is a persisted scope value and is kept as-is; see the module docstring.
    """
    return f"circle:{space_id}"


def parse_scope(scope: UsageScope) -> tuple[str, str | None]:
    """Split a Usage_Scope into ``(kind, space_id)`` where kind is
    ``"personal"`` or ``"circle"``.
...
# ---------------------------------------------------------------------------
# Seat tier resolution
# ---------------------------------------------------------------------------
#
# Contract, unchanged from the pre-migration helper:
#   - Returns the stored Seat_Tier value as a string
#     ("PLUS_SEAT" or "FREE_SEAT").
#   - Non-members and lookup failures resolve to "FREE_SEAT" so the request
#     is gated as Free without raising — authorization errors belong to the
#     per-route membership check, not the tier resolver.


async def read_seat_tier_for_user(user_id: str, space_id: str) -> str:
    """Return ``SpaceMember.seatTier`` for ``(user_id, space_id)``.

    Delegates to the migrated seat service, which owns this read and already applies the
    "FREE_SEAT on absence or failure" contract.
    """
    from src.domains.learning_spaces.services.seat_impl import get_seat_tier

    return await get_seat_tier(user_id, space_id)
...
# ---------------------------------------------------------------------------
# Database interface (injectable for testing / decoupling from the data layer)
# ---------------------------------------------------------------------------
...
class FeatureFlagService:
    ...
    def __init__(
        ...
        Args:
            ...
            tier_allowlists: Mapping of tier name → comma-separated
                ...
                allowlist keys were removed by Circle Reimagining; space
    ...
    async def _fetch_seat_tier(user_id: str, space_id: str) -> str:
        """Resolve ``SpaceMember.seatTier`` via the seat service.

        Returns ``"FREE_SEAT"`` for non-members or lookup failures.
        """
        return await read_seat_tier_for_user(user_id, space_id)
