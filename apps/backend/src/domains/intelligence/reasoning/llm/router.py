...
Ported from ``src/services/llm/router.py`` at ``4953972^``. The routing logic is unchanged.
Two things had to change, and both would have been silent or late failures if carried over verbatim:

- **``circle_id`` is now ``space_id``.** ``websocket_handler`` already calls this with
  ``space_id=`` (lines 471, 1586) because the Circle→Space rename landed on the caller first. A
  verbatim port would have raised ``TypeError: route_request() got an unexpected keyword argument
  'space_id'`` on the first real turn — after the learner's message had been saved.
- **``emit_ai_usage`` is called with ``space_id=``, not ``circle_id=``.** The current signature
  (``billing/services/usage_tracking.py``) takes ``space_id`` and absorbs everything else into
  ``**_kwargs``. Passing ``circle_id=`` would therefore have been accepted, dropped, and never
  logged, so space-scoped usage would have silently recorded as unattributed — with the call already
  wrapped in ``except Exception`` so nothing would surface.

The ``CostTrackerProtocol`` below is deliberately kept: the router depends on the two methods it
calls, not on the concrete ``CostTracker``, which is what lets the tests inject a fake.
...
from src.domains.intelligence.reasoning.llm.capabilities import TASK_CAPABILITY_MAP
...
from src.domains.intelligence.reasoning.llm.metrics import (
...
class LLMRouter:
    ...
    async def route_request(
        ...
        *,
        ...
        Args:
            space_id: The Space this request is scoped to, when ``usage_scope`` is a space
                scope. Named ``circle_id`` before the Circle→Space rename.
        ...
        for provider, model in candidates:
            ...
            try:
                ...
                # Space quotas and analytics stay isolated (Requirements
