...
- [ ] Tests: every operation in the §6.5 table deducts units. The behaviours are covered (`test_llm_metering.py`, 18 tests) but not the enumeration — nothing fails if a *new* call site forgets to pass `user_id`, and that is the gap through which an unmetered surface returns. A test that walks the call sites is worth more than another behavioural case.
