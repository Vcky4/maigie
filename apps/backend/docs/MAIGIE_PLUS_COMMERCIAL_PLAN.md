...
- [ ] **Read the meter.** Not a code item and the last thing standing between §6.7's model and measurement: every payer rate and consumption figure in the plan is still a guess, and the instrumentation to replace them has been live since Phase 3.
...
- [ ] **The 20%-of-backstop proactive sub-budget is not built.** Proactive AI is charged to the learner's window, which is rule 1 of Decision M, but nothing stops it consuming an allowance the learner was saving for a study session — which is the *rest* of rule 1. It needs a second counter on the month, and Decision R's order-of-magnitude test should be applied before adding one: this is a sub-budget of an existing meter rather than a new currency, so it is not the third meter Decision R forbids, but it is close enough to be worth arguing.
...
- [ ] Test that the proactive sub-cap cannot consume a session allowance — waiting on the sub-cap itself.
