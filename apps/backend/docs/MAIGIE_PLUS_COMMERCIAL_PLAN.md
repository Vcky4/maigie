...
- [ ] **Label the other call sites.** `operation` still defaults to `"unknown"` for the sites not yet converted. A reporting gap rather than a charging one — they are metered, just not attributed — and `"unknown"` is visible in the logs until a label arrives.
- [ ] **Make the chokepoint entitlement-aware and close drift 23.** `llm_resilient` still resolves a model from `registry._DEFAULTS` with no allowlist, so a free learner's quizzes, lessons and documents run the Plus model. Same chokepoint, same plumbing; Decision P sets the threshold.
- [ ] Shorten the retry/fallback chain. Now that retries are charged this is a cost decision as well as a reliability one, and up to nine billable calls for one operation is the number to argue with.
- [ ] Cache home guidance; set `thinking_budget` per operation class; redirect the four stragglers (`memory_impl.py:49`, `planning_impl.py:55`, `schedule_regen_impl.py:213`, `:227`).
