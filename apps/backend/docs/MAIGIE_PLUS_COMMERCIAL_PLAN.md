...
>
> Last reviewed: 2026-09-05 (revision 14)
...
- [ ] `plus_voice_30` in the catalogue, priced, and accepted by `POST /billing/passes/checkout` with the `403 VOICE_PACK_REQUIRES_PLUS` refusal. **Until this lands the product does not exist**, which is why `study_voice.routes.voice_exhausted_message` reads `PlanItem.purchasable` instead of offering a top-up: the copy shipped promising one before the rail existed, and a learner who ran out was sent nowhere.
