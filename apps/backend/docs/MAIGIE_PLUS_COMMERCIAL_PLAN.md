...
- [ ] **Label the call sites.** `operation` defaults to `"unknown"` so metering could land without touching 26 files in the same commit; the labels arrive per call site and `"unknown"` is visible in the logs until they do. Nothing is unmetered by this — it is a reporting gap, not a charging one.
- [ ] **The window is not enforced at the chokepoint.** `record_units` advances the counters and never refuses, so an exhausted learner is refused by the *next* gated operation rather than the current one — a window can be exceeded by at most one operation in flight, which is the price of measuring cost instead of estimating it. Decision L specifies accounting here and gating elsewhere; **what is missing is a cheap pre-flight refusal for a learner who is already over**, which would stop spend before it happens rather than after. Not built, and it is the gap most worth closing next.
- [ ] Delete `ESTIMATED_OPERATION_UNITS` — the last tabulated prices in the codebase, and its own docstring says Phase 3b deletes it. Three operations still read it.
...
- [ ] **Make the chokepoint entitlement-aware, and close drift 23.** `llm_resilient` picks a provider from `["gemini", "openai", "anthropic"]` and a model from `registry._DEFAULTS` — mostly `gemini-3.5-flash` — and consults no allowlist, so 26 of 27 AI surfaces run the Plus model for a free learner. The model-quality half of §6.3's paywall is currently true of chat alone. This is the same chokepoint and the same plumbing as the metering above, which is the argument for doing it here rather than sooner: a tier-aware model choice bolted on beside the meter would be a second place that decides what a learner is entitled to, and Decision B exists to prevent that. Two open sub-decisions: whether the split applies to every operation or only the expensive ones, and what Free's *second* candidate is — narrowing it to one model narrows its fallback to one model, and `gemini-2.5-flash-lite` ($0.10/$0.40, priced in both tables) has no registered adapter.
