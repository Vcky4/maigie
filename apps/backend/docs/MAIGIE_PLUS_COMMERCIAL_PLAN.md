...
> Status: **Phases 0–5 done on the backend, and §5.7 is fully complete as of 2026-09-05** — products, prices, webhooks, wallet methods, the Apple In-App Purchase key, the by-hand parity checks, and the removal of the retired Study Circle / Squad plan-code settings. A pass can be bought, verified and granted on all four rails. What remains before launch is the client surfaces (Phase 7 — web and mobile pass wallet, window/voice meters, points wallet, iOS StoreKit branch), the conversion wiring and drift-list cleanup (Phase 6), and the copy (Phase 8).
>
...
>
> **Phase 4 and Phase 5 are now built.** The `PlusPass` and `PlusPurchase` tables exist (migrations `071_plus_passes`, `075_points_ledger`), `pass_service` holds and activates passes, and `purchase_service` verifies and grants on every rail — Stripe one-time checkout, Paystack NGN charges, Google Play `purchases.products.get`, and Apple StoreKit JWS via `apple_service`. The one-time checkout exists (`POST /billing/passes/checkout`), points redeem into passes (`POST /billing/points/redeem`), and the store products have been created in Stripe, Paystack, Google Play and App Store Connect. **What is left is not backend money-path code, and §5.7 is now complete.** All four consoles are configured — products and prices, webhook endpoints and secrets, Stripe wallet methods, the Apple Paid Applications agreement, the Apple In-App Purchase key (distinct from the App Store Connect API key), and the §5.7.6 parity checks recorded 2026-09-05 — and the retired Study Circle / Squad plan-code settings have been removed from `config.py` and `.env.example` along with their `_assert_price_id_is_active` branches (§5.7.3). The frontier is the clients (Phase 7), the conversion/drift work (Phase 6) and the copy (Phase 8).
>
> ~~**Also outstanding from revision 9: the voice counters do not exist.**~~ **They exist as of Phase 3's close.** Live voice draws from `voiceSecondsRemaining` rather than from the unit window, the catalogue's voice figures read the voice allowance instead of dividing the window by a rate, Free is gated out of voice as a capability (closing drift 5), and `GET /billing/voice/balance` ships with the counter. `plus_voice_30` now has somewhere correct to land — `voiceSecondsPurchased`, which never expires — and Phase 5 owes it only a purchase rail.
>
> **Phase 3 is closed apart from reading the meter**, which is not a code item: `record_units` logs units per operation per learner, and replacing §6.7's estimates with a real distribution needs a month of traffic rather than a change.
>
...
> | Rev | Change | Where |
> | --- | --- | --- |
> | 15 | **Reconciled the document with the code and the consoles.** The header still described Phases 4 and 5 as the blocker with no `PlusPass` table, no checkout and no store products — all three false: `071_plus_passes` and `075_points_ledger` exist, `pass_service`/`purchase_service`/`points_service`/`apple_service`/`voice_service` are in `billing/services/`, and `/passes`, `/passes/checkout`, `/points/redeem` and the four verification endpoints are mounted. Phase 4b's checkboxes were all unchecked despite being built (migration, service, the `qualify_referrals`/`expire_points` Celery beats, `notify_expiring_grants`, the three endpoints and `tests/test_points.py`), and cited migration `063` where the real one is `075`. The Phase 4 voice-pack checkout item was unchecked despite `VOICE_PACK_REQUIRES_PLUS` being enforced in `routes.py`. §8's migration head said `062`; it is `076`. **§5.7's provider setup confirmed complete** across Stripe, Paystack, Google Play and App Store Connect — products, prices, webhooks and secrets, wallet methods, the Apple In-App Purchase key, and the §5.7.6 parity checks recorded 2026-09-05. The retired Study Circle / Squad plan-code settings were then removed from `config.py` and `.env.example` with their `_assert_price_id_is_active` branches, closing §5.7 entirely; the `410` path for retired plan ids survives via `DEPRECATED_PLAN_IDS`. | header, §5.7, §8, Phases 4/4b |
> | 14 | **Phase 5: the purchase rails — a pass can now be bought.** One shared seam, `purchase_service` (verify → persist a `PlusPurchase` → grant, idempotent on the unique `providerReference`; `409 PURCHASE_ALREADY_CLAIMED` for a cross-account token; refund-by-revocation), with each provider feeding it. Stripe: `mode: payment` one-time checkout for the two passes and the voice pack, `checkout.session.completed` → grant and `charge.refunded` → revoke (and a fix to a dispatch that passed the whole event where an event-type string was expected). Paystack: NGN one-off charges for all three passes including the NGN-only Term Pass, `charge.success` verified and attributed by metadata. Google Play: `purchases.products.get` verification at `POST /billing/purchases/google-play/verify`, RTDN voided-purchase revocation. Apple: `apple_service` verifies StoreKit JWS via Apple's `app-store-server-library` (new dependency — hand-rolling x5c chain validation on a money path was the alternative), `POST /billing/purchases/apple/verify` and `POST /webhooks/apple`. The voice pack requires an active Plus entitlement to buy on every rail (Decision R). **A store product must still exist in each console, and Apple is gated on the Paid Applications agreement** (§5.7); the rails are testable in the providers' sandboxes now. Activation into a running pass is Phase 7's client work. | Phase 5, §5.7, Decisions G/R |
> | 13 | **Phase 4b: points are a ledger a referral earns and a pass spends.** Migration `075_points_ledger`, `PointsLedgerEntry` and the `User.pointsBalance` cache, `points_service` (grant, balance, history, redeem, `expire_due`, `qualify_referral`), the daily qualification and nightly-expiry Celery tasks, and three endpoints — `GET /billing/points`, `POST /billing/points/redeem`, `GET /billing/referrals`. Points redeem into a pass through the existing `pass_service.grant(source="points")` seam with no `PlusPurchase` behind it, and the subscription is unreachable from redemption *by construction* — `POINTS_COST` holds only the two pass ids — rather than by a validation. Expiry is lazy on read (balance and FIFO both exclude expired grants) plus a nightly sweep that writes the explaining entries, so a stale sweep can never let dead points spend. Qualification counts distinct billable days from `UsageEvent` (an app open is not activity), and a 7-day expiry warning fires once per grant only when that grant alone can still buy a pass. **Passes still cannot be bought** — Phase 5's rail and §5.7's store products. | §6.9, Decision O |
> | 12 | **Phase 4's core: a learner can hold and activate a pass.** Migration `071_plus_passes`, `pass_service`, the two endpoints, the five-minute sweep, and the `_read_active_pass` seam filled — moved onto `resolve`'s existing joined read rather than costing a second query, which also made Decision E's two endings pure functions of columns. Decision E's allowance ending needed a third counter (`PlusPass.unitsUsed`) because the window and month both reset. Two figures in this document were stale and are corrected: §8's `unitsAllowance` still said 3 000 for the 5-hour pass, and §6.3's Term Pass row said "20 000/month", which over four months is $8.00 of COGS against $3.65 of net revenue. **Passes still cannot be bought** — that is Phase 5's rail and §5.7's store products. | §8, §6.3, Decisions A/C/D/E |
> | 11c | **The free reflection is deterministic, and every charge is itemised.** Decision M rule 2's open half turned out not to need the product decision it was waiting on: "silent depth" is an enforcement style and "no model call" is about who pays, so a measured summary satisfies both. What blocked it was `_fallback_summary` serving as both the error path and the free tier's only candidate copy. Free was spending *two* calls and `assemble(deep=False)` was discarding the reply. Separately, migration `070_usage_events` gives every charge a row, so §6.5's 27 estimates and Decision P's 500-unit threshold can be checked against the database instead of trusted — done before Phase 4 because the threshold is enforced now. | Decision M, §6.5, Decision P |
> | 11b | **Phase 3 closed: live voice has its own balance.** Migration `068_voice_balance`, `voice_service`, the §6.3 allowances on `Entitlement`, and `GET /billing/voice/balance`. Three departures from the item as written, each recorded in Phase 3: the sweep is replaced by re-deriving the grant on read (no job to fail, no interval in which an expired pass's minutes are still spendable); a third column `voiceSecondsPurchased` holds bought minutes that must never be reset; and the catalogue's voice figures move off the unit window, which retires "about N minutes" as an arithmetic artefact. Drift 5 closed with it — `study_voice` had no tier gate at all. | §6.3, drift 5, Phase 3 |
...
>
> Last reviewed: 2026-09-05 (revision 15)
...
5. **AI Voice Assistant / Live voice tutor as Plus-only.** `study_voice` contains no `check_capability`, no `require_premium`, no tier read except `billing_mode_for_tier`. Free gets the whole feature. → ~~**delete the ✗**~~ — **✅ closed with Phase 3's voice counter, and the marketing turned out to be right.** This item concluded the *copy* was wrong because the code did not enforce the claim. §6.3 then settled the product the other way and gave Free zero voice minutes, so the ✗ stays and the gate now exists (pre-flight on the voice balance, refusing a free learner with a capability message rather than an allowance one). Worth leaving visible: "the copy is wrong" and "the code is wrong" are indistinguishable from a drift audit, and which one you fix is a product decision rather than an engineering one. For paying learners the remaining tier difference is unchanged — wall-clock versus audio-only billing.
...
- [x] Enable **Apple Pay, Google Pay and Link** as payment methods in the dashboard. This is a checkbox, not an integration, and it is the correct place for the wallet APIs — §2 explains why using them inside the iOS app would be a guideline 3.1.1 rejection.
- [x] **Archive** the yearly Plus price and the three credit-pack prices. Archive rather than delete: Stripe keeps them referenceable, and archiving is what stops them being selectable.
- [x] Set the webhook endpoint to `POST /webhooks/stripe` for `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`, `charge.refunded`. **Copy the signing secret into `STRIPE_WEBHOOK_SECRET`** — an unset secret now refuses ingestion with a `503` rather than trusting the body (Phase 2a), so a missing secret fails loudly instead of granting tiers to anyone who posts.
...
- [x] Set the webhook to `POST /webhooks/paystack` and **copy the secret into `PAYSTACK_WEBHOOK_SECRET`**. The handler HMACs the body and now refuses an empty key rather than skipping the check.
- [x] **Delete** the Study Circle and Squad plan codes (§5.1) — done. The eight settings are removed from `config.py` and `.env.example` (`PAYSTACK_PLAN_STUDY_CIRCLE_MONTHLY/_YEARLY`, `PAYSTACK_PLAN_SQUAD_MONTHLY/_YEARLY` and the four Stripe equivalents), the `_plan_code_to_tier` branches were already gone (Phase 2b), and the four `_assert_price_id_is_active` price-id branches that read the Stripe settings are removed with them. The retired *plan ids* still answer `410` through `assert_plan_id_is_active` / `DEPRECATED_PLAN_IDS`, which is plan-id keyed and needs no setting. `test_subscription_catalog.py`'s deprecation assertions still pass.
...
- [x] **Delete, do not repurpose**: the `plus-yearly` base plan and the three `credit_pack_*` consumables, plus `GOOGLE_PLAY_BASE_PLAN_YEARLY` and the three `GOOGLE_PLAY_SKU_CREDIT_*` settings and the branches reading them. `config.py` no longer carries the yearly base plan or the credit SKUs — both are documented as withdrawn in their place. Nobody has bought any of them, so there is no RTDN history to decode. **Never rename an existing SKU into a new role** — a renamed SKU carries its old purchase history and its old type.
- [x] Point Real-Time Developer Notifications at the Pub/Sub topic feeding `POST /webhooks/google-play/rtdn`, and set `GOOGLE_PUBSUB_AUDIENCE` and `GOOGLE_PUBSUB_SERVICE_ACCOUNT_EMAIL`. The endpoint verifies the push OIDC token against Google's certs and checks the token's `email` — a Google-signed token proves Google minted it, not that our subscription sent it (Phase 2a).
- [x] Confirm the service account still has **View financial data** and Play Developer API access, which is what `purchases.products.get` and `purchases.subscriptions.get` authenticate as.
...
- [x] **Generate the In-App Purchase key — not the App Store Connect API key.** There are two Apple keys with nearly the same name, both issuing an issuer ID, a key ID and a `.p8`, and **they are not interchangeable**:
...
- [x] Set the **App Store Server Notifications V2** URL to `POST /webhooks/apple`.
- [x] Each product needs a **screenshot and review notes** before it can be reviewed. Trivial work that reliably surprises people on submission day.

#### 5.7.6 Parity checks, by hand, recorded with a date

Nothing tests these, because nothing in the repo can read a console. **Checked by hand and confirmed 2026-09-05.**

- [x] **The trial is 3 days in four places**: `config.TRIAL_DAYS_MAIGIE_PLUS`, the Stripe price's `trial_period_days`, the Play base-plan free trial, and the Apple introductory offer. Two of the four are consoles. `TRIAL_DAYS_CIRCLE_PLAN` stays at **7** — it is space-scoped (Decision F). *(2026-09-05)*
- [x] **Every NGN price matches §6.8** in Play, App Store Connect and `config.py`: ₦700 / ₦1 500 / ₦2 400 / ₦5 500, and ₦1 500 for the voice pack. *(2026-09-05)*
- [x] **Every USD price matches §6.1** in Stripe and both stores: $0.99 / $1.49 / $3.99 / $9.99. **No console anywhere still reads $4.99 or $2.49.** *(2026-09-05)*
- [x] **The Term Pass is unavailable outside Nigeria** in both stores, and `PlanItem.availability` keeps it out of `GET /plans/catalog` elsewhere (§6.1). A catalogue that advertises an unbuyable product is a defect this plan has already recorded once. *(2026-09-05)*
- [x] **Every pass and the voice pack are consumable** in both stores. Check it before the first purchase, not after — it cannot be corrected by editing. *(2026-09-05)*
- [x] **The two Apple keys are not confused.** `APPLE_KEY_ID` belongs to the **In-App Purchase** key, not the App Store Connect API key on the Expo account. The symptom of getting this wrong is authentication failures on transaction lookup while builds upload perfectly. *(2026-09-05)*
...
| Term Pass (NGN) | 4 000 units/window | **20 000 total** | ~114 total | separate allowance | $2.00 total |

> **This row said "20 000/month" and that was not survivable.** Over the Term Pass's four months it is 80 000 units — **$8.00 of COGS against $3.65 of blended net revenue**, a loss on every sale, and Decision Q's rule one says a product's ceiling COGS *is* its allowance cap × $0.0001. §6.8's margin table, which is where the NGN-only product actually lives, says 20 000 units at a 45% floor margin. The two statements could not both be true and the total is the coherent one, so `pass_service.PASS_PRODUCTS` implements 20 000 as the pass total. Found while building Phase 4, which needed a single number for `unitsAllowance`.
...
2. **Cadence follows entitlement, not the calendar.** Free gets discovery recommendations weekly, Plus nightly. Free weekly reflections are composed deterministically with no model call; the LLM narrative is the Plus version, which is what `feature_tier_service` already claims.

   ✅ **Done, and the composer is not the one named here.** This rule pointed at `weekly_summary.py`, which is deterministic but carries four figures — minutes, sessions, messages, streak — against `reflection_metrics.compute_metrics`' ten. The better composer was already inside `reflection_service`: the **Decision T2 skeleton** it builds before any model runs. `build_signals`, `build_subjects`, `build_rhythm`, `build_highlights` and `choose_actions` are all pure functions of the metrics, so the free reflection's content never needed designing — only its opening paragraph did, and `reflection_metrics.build_measured_summary` is that.

   **What actually blocked this for a phase was one string doing two jobs.** `_fallback_summary` was both the error path for a failed generation and the only candidate for free-tier copy, and those need different sentences — one string cannot be "here is your week" and "this did not work". So skipping the call would have told every free learner their reflection had failed, and the item read as blocked on a product decision it never needed. Split into `_error_summary` and `build_measured_summary`.

   **Free was spending two calls, not one, and keeping almost nothing from either.** The title/summary call ran for every tier, and the narrative call ran on `if deep or chosen` — which on Free is whenever any metric is actionable, since `_FREE_RECOMMENDATIONS = 1`. Then `reflection_narrative.assemble(deep=False)` **discarded the reply**: no signal prose, no patterns, no closing, no per-subject insight, and `opening` taken from `summary`. A free learner triggered an 8 192-token `THINKING_DYNAMIC` generation and kept one action's wording out of it. The gate is now `if deep`.

   Two things fell out of the split. `build_measured_summary` is also the better **Plus** failure path — a subscriber whose narrative fails now reads their real figures instead of an apology — and `_ACTION_COPY` gives each action id its own deterministic title and label, because "Keep going" was acceptable as a last resort after a failed call and is not acceptable as the free tier's only wording. `grounds` is published as the action detail on this path, which the model path deliberately never does: there is nothing to reword, and "Physics is at 40% (4 of 10 topics)" is a better detail line than any template, being the actual reason the action was chosen.
...
**The pack stacks, unlike a pass.** `plus_voice_30` is a balance, not an entitlement, so Decision D's redundancy refusal does not apply and a learner may hold and consume any number.

**And bought minutes do not expire — corrected against the implementation, which chose better than this decision originally did.** The first version of this paragraph said minutes "expire with the entitlement that was active when they were bought… for the same reason unused pass time is forfeited". The analogy was wrong. Unused pass *time* is forfeited because time is what the pass sold and it has passed; bought minutes are **inventory the learner paid for separately**, and the granted balance resets on renewal, so minutes sharing that column would be deleted on a billing date. Purchased seconds live in their own column, never reset, and are spent **after** granted ones so the perishable balance goes first. A learner who buys minutes and then stops subscribing keeps them: taking them back because they stopped paying for something else is un-selling a product. §8 has the mechanics.

## 8. Data model

This model landed across migrations `071_plus_passes` (passes, purchases, the `User` pass columns and Decision A's partial unique index) and `075_points_ledger` (the points ledger, `User.pointsBalance`, and `PlusPass.source` / `PlusPass.purchaseId` made nullable). The current head is `076_drop_notification_pushed_at`. The original plan reserved `063`, but four Phase 3 migrations (`067`–`070`) and the `072_drop_credit_tables` cleanup landed in between, so the real numbering runs ahead of this section's original prose.
...
| `unitsAllowance` | Integer | **2 000 \| 10 000 \| 20 000** — was written here as "3 000 \| 10 000", stale since revision 10 moved the 5-hour pass 3 000 → 2 000 (§6.3 and `entitlement_service` both say 2 000, and 3 000 is the inverted ladder that revision existed to fix). Snapshotted for the same reason as the duration (Decision E) |
...
**A `plus_voice_30` purchase adds 1 800 seconds to `voiceSecondsPurchased`** — a *third* column, and one this section originally did not have — and writes a `PlusPurchase` row with no `PlusPass` behind it, the first purchase in the plan that grants no entitlement.

> **Corrected against the implementation, which was right and this paragraph was wrong.** It used to say bought minutes land in the same counter and "inherit the expiry of whatever entitlement is already active, which is what makes them expire with it". Both halves were mistakes, and the first causes the second: a granted balance **resets** on renewal, so 30 bought minutes sharing that column would be **deleted at the next renewal** by the re-grant. A learner would pay $1.49 and lose it on their billing date.
>
> So purchased seconds are held separately, never reset, and **spent last** — granted seconds go first because those are the ones that expire, and draining the permanent balance while the perishable one sat unused would mean paying to lose minutes. They also survive the plan lapsing entirely: **someone who bought minutes and then stopped subscribing still owns them.** Confiscating them because they stopped paying for something else is taking a product back after selling it, and `available` therefore follows the *balance* rather than the tier for a lapsed learner. `add_purchased` exists so Phase 5's rail has somewhere correct to write; a top-up landing in the granted balance is the kind of mistake that looks right until a renewal.
...
- [x] **The voice counters ship, and voice is off the unit window.** Migration `068_voice_balance` adds `voiceSecondsRemaining`, `voiceSecondsPurchased` and `voiceAllowanceSourceId`; `entitlement_service` carries the §6.3 allowances (Free 0, Plus 60/month, 5-hour pass 10, 7-day pass 25, Term 60/month); `billing.services.voice_service` owns the balance; and `study_voice` charges seconds against it in place of `consume_credits`. `GET /billing/voice/balance` ships with the counter. 60 tests across `test_voice_balance.py` and a `TestVoiceIsADifferentMeter` scope guard.

  **There is no sweep, and the item above asked for one.** "The sweep must zero the balance when its source pass or subscription period ends" describes a job that can fail, and an interval between a pass ending and the job noticing during which its minutes are still spendable. The balance is re-derived on read instead: `voiceAllowanceSourceId` names what granted it, and when the entitlement stops naming the same source the granted balance resets. A renewal re-grants, a pass takes its minutes with it at the moment it stops being the active entitlement, and there is nothing to schedule. It is also the pattern this domain already used — `window_state` rolls the window over on read and `_subscription_lapsed` expires a subscription on read — so the sweep would have been the odd one out.

  **A third column the plan did not name: `voiceSecondsPurchased`.** `plus_voice_30` sells 30 minutes, and a re-grant that reset one balance would have deleted them at the next renewal. Purchased seconds are held separately, never reset, and **spent last** — granted seconds go first because those are the ones that expire. They also survive the plan lapsing entirely: a learner who bought minutes and then stopped subscribing still owns them, and confiscating them would be taking back a product after selling it. `add_purchased` exists now so that Phase 5's rail has somewhere correct to write; putting a top-up in the granted balance is the kind of mistake that looks right until a renewal.

  **Two figures moved off the window as a consequence.** `_voice_minutes_note` derived the catalogue's voice copy by dividing the *unit* window by the voice rate — so "about 15 minutes" was an arithmetic artefact rather than a promise, which is what the previous item flagged. It reads the voice allowance now, and the word "about" is gone with it. And `test_credit_service` asserted free voice was "about 2.5 minutes a window", which §6.3 had already retired to zero: that test now asserts the zero, and the free tier's note states voice as a **Plus capability** rather than quoting an allowance of nothing.

  **Drift 5 is closed in the same change.** `study_voice` had no tier gate anywhere, so the most expensive operation in the product was open to every free learner. The pre-flight now refuses on the voice balance, and refuses a free learner with a capability message rather than an allowance one — §6.3's argument is that voice is the clearest thing a pass sells, and "you have used your 0 minutes" sells nothing.

  Also, quietly: **the voice rate is no longer a charging rate.** `GEMINI_LIVE_UNITS_PER_MINUTE` survives only as the cost basis the margin tables quote. `GEMINI_LIVE_MIN_SESSION_UNITS` → `_SECONDS` and `GEMINI_LIVE_BILLING_MIN_CONSUME_CHUNK` → `_SECONDS`, because both had been wrong in *both directions* while denominated in a derived currency — the batching threshold was 0.3 seconds of audio under the old rate when it was meant to be 15. A threshold in seconds cannot drift when a price moves.
...
- [x] **`generate_grounded_content` is metered.** `GroundedResult.usage` carries the token counts and `resource_service` charges them through `llm_resilient.meter_usage`, which is now public for exactly this reason — grounding cannot go through the chokepoint, because the search tool has no fallback provider, so it would otherwise have to reimplement the exemption list and the never-raises guarantee. Charged **before** the empty-reply check: at ~1 600 units this is the most expensive operation in the product, and a measured run has already been seen spending 1 067 of them on reasoning before truncating. Charging on delivery would have made the dearest failure in the product free to us and invisible to everyone.
- [x] **The stragglers are done, two by redirection and one deliberately not.** `memory_impl` and `schedule_regen_impl` built their own clients and now call the chokepoint — `schedule_regen_impl` was a hand-rolled copy of `generate_content_json` (Gemini, fall back to OpenAI, strip fences, parse) minus the meter, the gate, the retry, the tier, the thinking budget and the truncated-JSON repair, and it **hardcoded `gemini-3.5-flash` for every learner**, which is drift 23 in miniature. About thirty-five lines of duplicated plumbing went with it, and the duplication is what let the cost hide.

  **`planning_impl` keeps its own client, and that is the decision rather than the exception.** It owns a 429 path that raises `PlanRateLimited` so a route can say "AI is temporarily busy" instead of failing, and the chokepoint would swallow the 429 into a generic `LLMUnavailableError` and lose that message. At ~225 units it sits below Decision P's threshold, so the tier gate the chokepoint would add is a no-op — which leaves the meter as the only thing that was actually missing, and it charges through `meter_usage` directly. `space_impl.py:1324` stays unmetered by Decision F.
...
- [x] **Total attempts are capped at `_MAX_TOTAL_ATTEMPTS = 4`, across every provider.** `_MAX_RETRIES` is per provider and said nothing about the sum, so three enabled providers meant nine billable calls for one generation. Four is three attempts on the primary plus one on a fallback: it keeps the retry that actually recovers a transient blank — the failure recovers on the second attempt, which is why the empty-reply handling exists — while refusing to fund a full walk of the chain. **Fixed while dormant on purpose**: the bug returns with the next provisioned key, and it returns as a bill rather than as an error. Three tests, including one that the cap must not clip the primary's retries.
- [x] The dead `CREDIT_COSTS` entries are gone — the whole table went with Phase 3, so there was nothing left to delete. The surviving `ai_course_generation` strings are a *feature label* in `usage_tracking`, not a price.
- [x] **Home guidance is cached, and it was the dearest operation per learner per month in the product.** It got there by being cheap: ~140 units a call is nothing, and firing on *every* home load is ~$2.10/month per active learner — more than a Plus subscription's whole margin. It goes through `narrative_cache` now, as `growth_service` and `goal_insight_service` have since Phase 6.5.

  **The key is `_build_llm_context(state)` — the exact string the prompt is built from**, not a chosen subset of fields, so if the context is byte-identical the guidance would be too. Two properties of that string are load-bearing and both are tested: it carries **no clock**, without which every load would be a miss and the cache would cost a write per read while saving nothing — the defect that defeats prefix caching on the chat path — and it *does* carry the day count and the due-flashcard count, so guidance refreshes daily and cannot be stale about what the learner has. The context is passed *into* the composer rather than rebuilt, because a cache that stores output produced from a different input than it is keyed on is worse than no cache. 14 tests.
- [x] **`thinking_budget` set on the operations that were still defaulting**, per Phase 0's three-class split. `home_guidance`, `plan_generation` and `schedule_regeneration` are all **bounded** — each assembles supplied figures into a fixed shape with the output word-bounded, so there is nothing to reason about beyond wording, and the provider default on a *thinking* model is an unbounded reasoning allowance for a paragraph. Both memory operations are **off**: they restate a transcript as fields, and memory is exempt from charging on principle (§6.6), so every reasoning token it spent was COGS with no revenue and no allowance behind it — the worst combination available. The genuine-reasoning class keeps the dynamic default deliberately; this replaces the withdrawn `max_tokens` audit, and lowering a ceiling would have re-broken five separately-diagnosed truncations.
- [x] **The call sites are checked by walking the source, not by behaviour.** `test_every_generation_is_metered.py` parses `src` and asserts every generation call passes `user_id` and `operation`, naming the file and line when one does not. This is the only test in the set that fails for a call site nobody wrote a test for, and that is the point: a call without `user_id` is *silently free*, so the surface goes unmetered with every existing test green. It is how the product reached 26 unmetered call sites in the first place — one omission at a time, never a decision.

  It also guards the hole the `user_id` check cannot see: **a module that builds its own client is not an unlabelled call site, it is an invisible one.** `TestNothingBypassesTheChokepoint` allows only the adapter package, `planning_impl` (the 429 exception) and `space_impl` (Decision F) to reach a provider SDK, so the next module that tries appears in a diff rather than in a bill. Both guards were mutation-tested: a call site with the kwargs removed and a module reaching the SDK directly each fail with the offending line named.

  Writing it found one thing about itself worth keeping: the Gemini SDK's own method is *also* called `generate_content`, so matching on the bare name flagged the adapters. Distinguished structurally — our helpers are called bare or on a module alias, the SDK form is a nested attribute chain — rather than by excluding paths, which would have quietly stopped covering new adapters.
- [x] **`scripts/read_the_meter.py` — the code half is done; running it against production is not a code item.** Read-only aggregates over the usage columns and `LlmCostRecord`, written as a script for the same reason `count_legacy_commercial_state.py` was: a number nobody can re-derive becomes a number somebody remembers wrongly.

  **It found something on its first run, and the finding is about how to read it.** Eleven `free` calls against `gemini-3.5-flash` in the last 30 days — which is exactly the defect Decision P exists to prevent — all dated 29–31 August, all *before* the allowlist was narrowed on 2026-09-02. Historical, not live. The script now says so in its own output, because a script that cries wolf about its own history is one people learn to ignore.

  Two things it could not tell you, both stated in its output. **The payer rate**, because there are no payers: every revenue figure in §6.7 and §6.11 rests on 6% and the mix in §6.8, and none of it is measurable until Phase 5 ships a checkout. Costs are now measured; revenue is not, and the model is only half checked. And **where the units went, per operation** — which the next item closes, so the script now aggregates `UsageEvent` and reports each operation's measured units against Decision P's threshold. What replaces that caveat in its output is narrower: the per-operation table ranks surfaces by spend but cannot say what a surface *would* cost at §6.7's forecast volume, because that needs the payer mix too.
- [x] **One row per metered operation — an addition, not an unfinished requirement.** Decision L asks for cost to be measured rather than tabulated, and it is; nothing in Phase 3 asked for per-operation *persistence*. Migration `070_usage_events` and `billing.db_models.UsageEvent`, written from `record_units`.

  **Done before Phase 4 rather than after, because Decision P's threshold is live now.** The split has been enforcing a 500-unit line against §6.5's estimates since Phase 3b, and until this table existed no query could contradict them — so the paywall was calibrated on numbers nothing checked, and would have stayed that way for however long Phases 4 and 5 took. `read_the_meter.py` now prints units per operation with the side of the threshold each one *measures* on, which is the check: an operation measuring above the line but absent from `QUALITY_SPLIT_OPERATIONS` is serving Plus the cheap model, and one measuring below it but present is buying a dearer model than the split meant to pay for.

  Four decisions worth recording. **`units` without the token counts it came from** — `units_for_tokens` has applied the rate already, and keeping both invites the two disagreeing, which is the failure this denomination exists to avoid. **`model` kept**, because under Decision P the same operation costs different units on different tiers, so which model ran is part of the answer rather than a restatement of it. **No `userTier` column**, since the tier is derivable from the model and a denormalised copy would be a second opinion about entitlement (Decision B). **No foreign key to `User`**: a deleted learner's spend still happened, and a cascade would quietly rewrite history to make the cost model look better than it was.

  It is a write on every generation, as the item warned. Mitigated by sitting *outside* the counter update's `try` and after it: the counters are the learner's accounting and the row is our analytics, so a failed insert cannot cost a learner their window reset, and a failed counter update still leaves the row as the only evidence of what was served. Both directions are tested.
...
- [x] Both proactive tasks are metered — they charge through the chokepoint like everything else, since Phase 3b put every generation on it.
- [x] **The 20%-of-backstop proactive sub-budget is built.** Migration `069_proactive_sub_budget` adds `usageMonthProactiveUnitsUsed`, and **it is one column rather than a second currency**: every proactive unit advances it *and* `usageMonthUnitsUsed`, so it is a category tag on the month's spend. That is what keeps it clear of Decision R, which forbids a new currency with its own counter rather than the same currency asked a second question — a background task cannot dodge the month by being proactive, nor the sub-cap by being ordinary. The cap is derived from `Entitlement.monthly_backstop` rather than stored, so an upgrade raises it immediately.

  **Marked by a context variable rather than a parameter**, because of the call depth: a task reaches a provider through a service that calls the chokepoint several frames down, and threading a flag would mean widening every signature in between — each one existing only to pass it on, and each a place to forget it. Forgetting fails silently, by charging the month without the sub-budget. `proactive_scope()` is entered **per learner** rather than around a batch, so one learner's accounting cannot leak into the next if a batch raises midway, and it resets on the exception path.

  Charged **and** gated: `has_proactive_headroom` runs after `has_headroom`, in that order deliberately — an exhausted learner should get the window message, because "out of allowance" is a truer statement of their position than "the background budget is spent".
- [x] **Discovery cadence follows entitlement: Free weekly, Plus nightly.** Enforced in `discovery_service._cadence_allows` rather than by a second Celery schedule, because a second beat entry is a second thing that can drift from the tier it tracks — and a tier can change between two runs of one schedule. It **fails open**: the cost of one extra recommendation is ~150 units, and the cost of the opposite mistake is a learner whose discovery feed silently stops. The gate runs *before* `delete_old_recommendations`, which is destructive — returning early after it would leave a free learner with nothing for six days rather than with last week's set, and that reads as the feature failing rather than as a cadence.
- [x] **Free reflections are composed with no model call.** The blocker was copy rather than plumbing, as this item said — but it was one string doing two jobs, not a product decision.

  **"Silent depth" and "no model call" are not different products.** The item flagged them as a conflict to settle. They answer different questions: §6.6's "silent depth" is an *enforcement style* — the free learner gets a shallower artefact with no `403` and no notice — while Decision M rule 2 is about who pays for a background generation. A deterministic summary is both silent (nothing is refused) and free (nothing is generated), and §6.6's own background row already read "deterministic, no LLM". Nothing had to be decided.

  **What blocked it was `_fallback_summary` serving as both the error path and the free tier's only candidate copy.** Those need different sentences, so removing the call would have published *"the narrative could not be generated this time"* to every free learner. Split into `_error_summary` (the narrow case where there are no figures at all) and `reflection_metrics.build_measured_summary`.

  The content needed no designing: the **Decision T2 skeleton** was already there and is richer than the `weekly_summary.py` this rule pointed at — ten measured fields against four. `build_measured_summary` composes two sentences from them, effort then outcome, with an honest "a quiet week" for a learner who did not study rather than a paragraph of zeros.

  **Free was spending two calls and keeping almost nothing.** The title/summary call ran for every tier, and the narrative call ran on `if deep or chosen` — non-empty for any actionable metric, since `_FREE_RECOMMENDATIONS = 1`. Then `assemble(deep=False)` discarded the reply entirely. The gate is now `if deep`. `_ACTION_COPY` supplies each action id a deterministic title and label, because "Keep going" was a fair last resort after a failed call and is not fair as the free tier's only wording.

  Two payoffs beyond the saving. A Plus learner whose narrative fails now reads their measured figures instead of an apology — the same function, both tiers. And the free reflection keeps its one recommendation, worded from the `grounds` the service chose it on, so it stays actionable rather than becoming a page of figures.
- [x] **Dormancy stop, at the query rather than in the loop.** `list_active_profiles(active_since=...)` joins `User` and filters on `usageWindowStartedAt` — the timestamp the meter maintains, which advances only when a usage window opens and therefore means "did something that cost us money". That is the right question for deciding whether to spend more. Applied to the two tasks Decision M names and **only** those: `daily_plan`, `behaviour` and the snapshot services fan out over the same profiles but call no model, so filtering them would change product behaviour with no cost to justify it.

  A `NULL` counts as dormant, deliberately — Principle Five says momentum is designable and there is none before the learner has done anything, and onboarding auto-setup covers the fresh case while being exempt from charging on purpose. Seven days rather than one or thirty because it has to survive an ordinary gap: a learner who studies at weekends is not dormant on a Wednesday.
- [x] Tests: `test_background_ai_budget.py`, 12 assertions across the cutoff, the fan-out actually passing it, the cadence in both tiers, a naive timestamp not crashing the gate, failing open, and nothing destructive running for a learner who is not due.
- [x] Tests for the sub-cap: the derived cap, no cap without a backstop, an upgrade raising it, both counters advancing together, an ordinary call leaving the proactive counter alone, the scope reaching the meter, and the scope resetting when its block raises. In `test_llm_metering.py` rather than the background-AI file, because they need the chokepoint fixtures — and one of them deliberately declines the `metered` fixture, since that fixture replaces `record_units` with a recorder and this assertion is about what the real function writes.

### Phase 4 — Passes

- [x] **Migration `071_plus_passes`** (not `063` — four migrations landed while this was written, three of them Phase 3's): `PlusPass`, `PlusPurchase`, `activePlusPassId` / `activePlusPassExpiresAt` / `appleOriginalTransactionId` / `appleProductId` on `User`, and Decision A's partial unique index.
- [x] SQLAlchemy models in `billing/db_models.py`.
- [x] **`pass_service.py`**: `grant`, `activate`, `list_passes`, `expire`, `revoke`. Activation snapshots duration and allowance onto the row, caches the pass on `User`, resets the usage window and invalidates the entitlement memo — the last of those being the same defect `trial_service.start_trial` fixes for trials, since `activate` resolves the entitlement at the top and would otherwise leave the rest of the request seeing a free learner.

  `grant` takes `duration_minutes` and `units_allowance` overrides so §6.8's NGN allowances are a property of the purchase rather than a currency branch in every reader. Decision D's three refusals carry **three different messages**, because a subscriber, a trialling learner and someone mid-pass are in three different situations and only one of them is their own doing.
- [x] **The `_read_active_pass` seam is filled, and moved.** It took a `user_id`, which would have meant a second query on the hottest path in the domain; the pass now arrives on the same joined read as the tier and the trial, since Decision C's `activePlusPassId` makes that join a primary-key lookup matching nothing for the 1 205 accounts holding no pass. `_active_pass` is therefore a pure function of columns, which is what makes Decision E's two endings testable without a database.
- [x] **Decision E's allowance ending is real, and it needed a third counter.** `PlusPass.unitsUsed`, advanced by `record_units` when a pass is what is granting Plus — the window rolls every five hours and the month on the 1st, while a pass total has to survive both. Advanced with `UPDATE … SET unitsUsed = least(unitsUsed + n, unitsAllowance)` rather than read-modify-write, because voice bills every couple of seconds and concurrent charges on one pass are the normal case; a read-modify-write would under-charge by exactly one operation each time. Clamped so a client never sees more than 100% of the figure it was sold.
- [x] `GET /billing/passes`, `POST /billing/passes/{id}/activate`. The listing returns ended passes too — "what happened to my pass" is a question about one that ended — and is the iOS restore path (Decision G).
- [x] **Celery beat: `billing.sweep_expired_passes` every 5 minutes**, and worth being precise about what it is for. Expiry is already true on read, so this is not the mechanism: it writes `status` and `endedReason` so the inventory has a durable answer, and it **tells the learner**, which is the part a lazy read cannot do. The five-minute interval is chosen for the notification's usefulness, not for correctness. It checks the clock before the allowance, so a pass that ran out of both is `expired` — its time was up regardless, and a learner told "you used it all" about a pass that simply ended would rightly disagree. **It does not touch the voice balance, and this line used to say it should.** The instruction was "the same sweep zeroes `voiceSecondsRemaining` when the entitlement its `voiceAllowanceSourceId` names ends" — which describes a job that can fail, and an interval between a pass ending and the job noticing during which its minutes are still spendable. Phase 3 built the lazy alternative instead: when the entitlement stops naming the stored source, the next read discards the granted balance. A renewal re-grants, a pass takes its minutes with it the moment it stops being active, and nothing is scheduled. **Anyone adding voice to this sweep would be fighting `voice_service.resolve`, not helping it.** It is also the pattern this domain already used — `window_state` rolls the window on read, `_subscription_lapsed` expires a subscription on read — so a sweep would have been the odd one out.
- [x] `voice_service.py` — built in Phase 3 as `read_balance`, `spend` and `add_purchased`. `add_purchased` writes to the never-resetting column rather than inheriting an expiry, for the reason in Decision R.
- [x] `GET /billing/voice/balance`.
- [x] `plus_voice_30` in the catalogue, priced (`PRICE_CENTS_PLUS_VOICE_30`, `PRICE_NGN_PLUS_VOICE_30`), and accepted by `POST /billing/passes/checkout` with the `403 VOICE_PACK_REQUIRES_PLUS` refusal enforced at the endpoint before a session is created. `study_voice.routes.voice_exhausted_message` reads `PlanItem.purchasable` instead of offering a top-up: the copy shipped promising one before the rail existed, and a learner who ran out was sent nowhere.
- [ ] Notifications: pass activated (with expiry time), 30 minutes remaining, pass ended. **Voice: at 5 minutes remaining, carrying the pack as the action** — the one notification in the plan allowed to name a purchase, because a learner mid-session about to lose the tutor needs the option before the silence rather than after. **Ship it with the rail, not before**: it must read `PlanItem.purchasable` the way `voice_exhausted_message` does, or it becomes a push notification advertising something unbuyable.
- [x] Tests: `tests/test_plus_passes.py`, 31 assertions. The one-active invariant asserted as a *partial* unique index on the model rather than through a simulated race — a non-partial index would forbid holding two passes in inventory at all, which is the opposite of Decision A; `PASS_REDUNDANT` for each of the three reasons and that all three read differently; both of Decision E's endings including the overshoot case; that a refused activation never opens a session, which is the structural version of "the learner keeps the pass"; and a Decision F scope guard asserting `pass_service` contains no space vocabulary at all.

  **One test found a live defect on first run.** `plus_pass_term` was in `VOICE_SECONDS_BY_PASS_PRODUCT` but missing from `WINDOW_ALLOWANCE_BY_PASS_PRODUCT`, so a four-month Term Pass would have run on the 5-hour pass's 2 000-unit window. The fallback that caused it is correct — an unknown product falls to the *smallest* allowance, because under-granting is a support ticket and over-granting is COGS — which is exactly why a missing row reads as working software. Three tables keyed by the same ids now have a test that says so.
- [ ] Remaining Phase 4 tests, which need the rails: that a voice pack stacks where a pass refuses, that a purchase without entitlement is refused rather than banked, and that a voice pack creates no `PlusPass` row. `pass_service.is_pass_product` exists for the rail to route on, and excludes `plus_voice_30` deliberately. **Voice pack: it stacks where a pass refuses** (two packs credit 3 600 seconds, two passes give `409`), a purchase without entitlement is refused rather than banked, **bought minutes survive both a renewal and the plan lapsing entirely**, and **a voice pack does not create a `PlusPass` row**. Scope guard: an activated pass does not change any space-scoped read.

### Phase 4b — Points (§6.9, Decision O)

Depends on Phase 4 and on Phase 3b, which is what makes "active" measurable.

- [x] Migration `075_points_ledger` (not `063` — it landed well after Phase 4's `071`): `PointsLedgerEntry`, `User.pointsBalance`, `PlusPass.source`, `PlusPass.purchaseId` made nullable. Constants: `POINTS_PER_QUALIFIED_REFERRAL = 100`, `POINTS_EXPIRY_DAYS = 60`, `POINTS_COST = {"plus_pass_5h": 100, "plus_pass_7d": 250}`.
- [x] `points_service.py`: `grant(user_id, points, kind, source_ref)` (idempotent on the unique index), `balance(user_id)`, `redeem(user_id, product_id)` → `pass_service.grant(source="points")`, `expire_due()`. Redemption spends the oldest live grant first and writes one negative entry per grant it consumes, so the ledger explains itself.
- [x] **Qualification job**: a referred learner is qualified on their **7th distinct day with a billable operation** — `billing.qualify_referrals` (`workers/billing_tasks.py`) walking pending `signup` referrals through `points_service.qualify_referral`. Evaluated from the usage records Phase 3b creates, not from `lastLoginAt`. Daily Celery beat at 02:00, idempotent, granting once per referred learner.
- [x] Celery beat: `billing.expire_points` nightly (02:30), writing the negative entries via `points_service.expire_due`. Lazy expiry on read as well, so a stale sweep can never let expired points be spent (the same belt-and-braces as the pass sweep, Decision E).
- [x] Notify once, 7 days before the oldest grant expires, **and only if that grant alone can still buy a pass** — `points_service.notify_expiring_grants`, fired by `expire_points_task`. No notification for an unspendable remainder.
- [x] `GET /billing/points`, `POST /billing/points/redeem`, `GET /billing/referrals`. The redeem endpoint accepts the two pass ids and has no subscription branch to remove later (Decision O).
- [ ] Monitoring: qualified referrals per referrer per week, and total live points as a forecastable COGS liability. There is no cap, so the alert is the control. **Still open — the jobs run, but no dashboard or alert reads them yet.**
- [x] Tests (`tests/test_points.py`): 100 points buys a 5-hour pass and leaves zero; 249 points cannot buy the 7-day pass; a grant 61 days old cannot be spent even if the sweep has not run; redemption is FIFO across three grants with different expiries; the same referred learner cannot grant twice; **six days of activity grants nothing and the seventh grants exactly 100**; seven logins with no billable operation grant nothing; **no request can produce subscription time from points** — assert on the redeem endpoint's accepted product set.
...
- [x] **§5.7.1–§5.7.5 are complete before any verification code is written.** Every server task below needs the product ids to exist, and the Apple prerequisites (§5.7.5) need a **Paid Applications agreement with banking and tax**, which is not an engineer's to supply and is the single item most likely to add a week. Done as of 2026-09-05.
- [x] **§5.7.6's parity checks are done by hand and recorded with a date** (2026-09-05). Nothing in the repo can read a console, so the trial being `3` in four places and every NGN price matching §6.8 are checks a person performs or nobody does.

Server rails:

- [x] **Stripe**: one-time Checkout for the two USD passes and the voice pack (`mode: payment`), the new `$9.99` subscription price. `checkout.session.completed` → `PlusPurchase` → `pass_service.grant`, **or `voice_service.credit` for `plus_voice_30`**, which grants seconds and no entitlement (Decision R). **The Term Pass has no Stripe rail** and must not acquire one (§5.7.1).
- [x] **Enforce `VOICE_PACK_REQUIRES_PLUS` at the checkout boundary on every rail**, not in the client. A cached catalogue can reach a Play or StoreKit purchase sheet, so a store purchase of `plus_voice_30` by a learner with no entitlement must be **verified, refunded-by-revocation and not credited** rather than silently banked — the one case in this plan where a valid store receipt is deliberately not honoured, and it needs a test.
- [x] **Paystack**: NGN one-time charges for **all three** passes including the Term Pass, from the `PRICE_NGN_*` settings; extend `handle_paystack_webhook` to attribute a one-off `charge.success` to a pass grant rather than a subscription. **Set `PRICE_NGN_PLUS_PASS_7D = 150_000` and add `PRICE_NGN_PLUS_PASS_TERM = 550_000` first** — the former still holds the retired ₦1 800 and the latter does not exist, and for a one-off charge nothing overrides a wrong amount (§5.7.3).
...
- [x] **Apple** (new domain code): `apple_service.py` — App Store Server API client, JWS verification of `signedTransactionInfo` against Apple's root CAs, `POST /billing/purchases/apple/verify`, `POST /webhooks/apple` handling `DID_RENEW`, `EXPIRED`, `REFUND`, `REVOKE`, `CONSUMPTION_REQUEST`. Config: `APPLE_ISSUER_ID`, `APPLE_KEY_ID`, `APPLE_PRIVATE_KEY`, `APPLE_BUNDLE_ID`, `APPLE_ENVIRONMENT`.
- [x] Idempotency and abuse tests: replayed token grants nothing; token bound to user A rejected for user B with `409`; refund revokes an active pass mid-run. Run the set against **both** stores, not just Play — Apple's `REVOKE` and Google's voided-purchase RTDN are different shapes reaching the same revocation path.
