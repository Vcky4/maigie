...
- [x] **`scripts/read_the_meter.py` — the code half is done; running it against production is not a code item.** Read-only aggregates over the usage columns and `LlmCostRecord`, written as a script for the same reason `count_legacy_commercial_state.py` was: a number nobody can re-derive becomes a number somebody remembers wrongly.
  ...
  **It found something on its first run, and the finding is about how to read it.** Eleven `free` calls against `gemini-3.5-flash` in the last 30 days — which is exactly the defect Decision P exists to prevent — all dated 29–31 August, all *before* the allowlist was narrowed on 2026-09-02. Historical, not live. The script now says so in its own output, because a script that cries wolf about its own history is one people learn to ignore.
  ...
  Two things it cannot tell you, both stated in its output. **The payer rate**, because there are no payers: every revenue figure in §6.7 and §6.11 rests on 6% and the mix in §6.8, and none of it is measurable until Phase 5 ships a checkout. Costs are now measured; revenue is not, and the model is only half checked. And **where the units went, per operation** — `record_units` logs the `operation` label and advances two aggregate counters, but writes no per-operation row, and `LlmCostRecord` has provider, model, tier and cost but no operation and is written from the chat path only. So §6.5's 27 estimates cannot be checked against measurement from the database, only from logs, which are not aggregated and do not persist.
- [ ] **One row per metered operation — an addition, not an unfinished requirement.** Decision L asks for cost to be measured rather than tabulated, and it is; nothing in Phase 3 asked for per-operation *persistence*. This item exists because reading the meter revealed the gap: §6.5's 27 estimates cannot be checked against measurement from the database, only from logs, which are not aggregated and do not persist. It is the first place to look when the next cost question is "which surface". Not urgent, and not free — it is a write on every generation.
...
- [ ] **Free reflections still call the model, and the blocker is copy rather than plumbing.** The `deep` branch at `reflection_service` changes prompt *depth*, not whether a model runs, and `_FREE_RECOMMENDATIONS = 1` means a free learner with any actionable metric has a non-empty `chosen` and gets the narrative call too — a comment there claimed free spends no call, which was wrong and is now corrected. Making the free version deterministic needs a real composer: **`_fallback_summary` is an error message** — *"the narrative could not be generated this time"* — so it cannot serve as the free tier's normal copy without telling every free learner their reflection failed. `weekly_summary.py` has the honest aggregates to build from.

  Worth settling when it is picked up: §6.6 describes this gate as "silent depth" and Decision M asks for no model call at all. Those are different products, and the second is a visible change to what a free learner reads.
