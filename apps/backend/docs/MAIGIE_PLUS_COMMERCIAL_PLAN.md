...
>
> One gap remains inside the closure, and it is on the cheap side: a fallback to OpenAI or Anthropic consults no allowlist, so a free learner whose Gemini attempts all fail gets `OPENAI_DEFAULT_MODEL`, which may cost more than the Plus model the split was avoiding. Left as one decision point rather than two — a second tier map for the fallback providers is the mistake drift 23 is a record of — and the fix if fallbacks stop being rare is a cheaper fallback model, not a second allowlist.
...
- [ ] Shorten the retry/fallback chain. Three attempts × three providers is up to **nine billable calls for one operation**, which is a cost decision now and not only a reliability one. It has also acquired a second reason: a fallback leaves the Gemini path, and the quality split only exists there, so a free learner falling through to `OPENAI_DEFAULT_MODEL` gets a model no allowlist was consulted about.
