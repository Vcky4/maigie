...
try:
    ...
    if "72 bytes" in error_msg or "__about__" in error_msg:
        ...
        check("✓ Password hashing utilities available (bcrypt version detection issue)", True, warning=True)
...
try:
    ...
    try:
        ...
        check("✓ Google provider has required attributes", 
              hasattr(google_provider, "authorize_url") and 
              hasattr(google_provider, "access_token_url"))
    except Exception as e:
    ...
    try:
        ...
        check("✓ GitHub provider has required attributes",
              hasattr(github_provider, "authorize_url") and
              hasattr(github_provider, "access_token_url"))
    except Exception as e:
...
try:
    ...
    check("✓ SecurityHeadersMiddleware has dispatch method",
          hasattr(SecurityHeadersMiddleware, "dispatch"))
    check("✓ LoggingMiddleware has dispatch method",
          hasattr(LoggingMiddleware, "dispatch"))
...
try:
    ...
    check("✓ OAuth authorize route available", any("/oauth" in path and "authorize" in path for path in route_paths))
    check("✓ OAuth callback route available", any("/oauth" in path and "callback" in path for path in route_paths))
...
try:
    ...
    check("✓ OAUTH_GOOGLE_CLIENT_SECRET setting exists", hasattr(settings, "OAUTH_GOOGLE_CLIENT_SECRET"))
    check("✓ OAUTH_GITHUB_CLIENT_ID setting exists", hasattr(settings, "OAUTH_GITHUB_CLIENT_ID"))
    check("✓ OAUTH_GITHUB_CLIENT_SECRET setting exists", hasattr(settings, "OAUTH_GITHUB_CLIENT_SECRET"))
