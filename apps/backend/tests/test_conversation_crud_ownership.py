    ...
    with patch.object(
        ...
        with pytest.raises(NotFoundError):
            await conversation_service.get_conversation(
                session_id="session_1", user_id="user_1"
            )
...
async def test_archive_checks_ownership_before_updating():
    ...
    with (
        ...
        patch.object(
            conversation_service.intelligence_repo, "update_chat_session", update
        ),
    ):
...
async def test_history_checks_ownership_and_returns_the_owned_thread():
    ...
    with (
        ...
        patch.object(
            conversation_service, "get_session_factory", lambda: lambda: session
        ),
    ):
